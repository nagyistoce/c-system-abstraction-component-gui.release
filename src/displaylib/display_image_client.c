...
IMAGE_PROC( Image,BuildImageFileEx) ( PCOLOR pc, _32 width, _32 height DBG_PASS)
{
   _32 ResultID;
   _32 Result[5];
   _32 ResultLen = 20;
...
IMAGE_PROC( Image, LoadImageFileEx ) ( CTEXTSTR filename DBG_PASS )
{
   if( !ConnectToServer() ) return NULL;
   {
   PMyImage image = NULL;
...
IMAGE_PROC( void,UnmakeImageFileEx) ( Image pImage DBG_PASS )
{
   PMyImage MyImage = (PMyImage)pImage;
   if( !ConnectToServer() ) return;
...
IMAGE_PROC( void,BlatColor)     ( Image pImage, S_32 x, S_32 y, _32 w, _32 h, CDATA color )
{
	PMyImage MyImage = (PMyImage)pImage;
...
IMAGE_PROC( Font,GetDefaultFont) ( void )
{
   ...
   if( !g.DefaultFont )
   {
        ...
        && (Result == (SERVER_SUCCESS|MSG_GetDefaultFont) ) )
      {
         ...
         g.DefaultFont->font = *(Font*)(ResultBuffer+0);
         g.DefaultFont->data = (Font)Allocate( ResultLength - 4 );
...
IMAGE_PROC( _32 ,GetFontHeight)  ( Font font )
{
	_32 result[2];
...
IMAGE_PROC( void,PutStringFontEx) ( Image pImage
                                 , S_32 x, S_32 y
                                 , CDATA color, CDATA background
                                 , CTEXTSTR pc, _32 nLen, Font font )
{
   PMyImage MyImage = (PMyImage)pImage;
   PMYFONT MyFont = (PMYFONT)font;
   if( !ConnectToServer() ) return;// 0;
      ...
      send_accept_only:
         ...
         else
         {
               ...
               ( Responce == ( MSG_AcceptTransferredFont | SERVER_SUCCESS ) ) )
            {
               ...
               resultfont->font = *(Font*)(ResultBuffer+0);
               resultfont->data = (Font)Allocate( resultlen - 4 );
...
IMAGE_PROC( void, OrphanSubImage )  ( Image image )
{
   PMyImage MyImage = (PMyImage)image;
   if( !MyImage )
      return;
...
IMAGE_PROC( void, SetImageAuxRect )( Image image, P_IMAGE_RECTANGLE pRect )
{
   PMyImage MyImage = (PMyImage)image;
   if( !MyImage )
      return;
...
IMAGE_PROC( void, GetImageAuxRect )( Image image, P_IMAGE_RECTANGLE pRect )
{
   PMyImage MyImage = (PMyImage)image;
   if( !MyImage )
      return;
...
IMAGE_NAMESPACE
IMAGE_PROC( Font, RenderScaledFontData )( PFONTDATA pfd, PFRACTION w, PFRACTION h )
{
...
IMAGE_PROC( Font, RenderFontFileEx )( CTEXTSTR file, _32 width, _32 height, _32 flags, P_32 nFontDataSize, POINTER *pFontData )
{
	struct result_tag {
		RENDER_FONTDATA prd;
		Font font;
		...
		&& ( responce == (MSG_RenderFontFileEx | SERVER_SUCCESS) ) )
	{
      ...
      //LogBinary( result, ResultSize );
		if( ResultSize > 4 )
		{
			...
			return *(Font*)(((char*)result) + ResultSize - 4);
		}
	}
...
int CPROC ClientGetFontRenderData( Font font, POINTER *fontdata, _32 *fontdatalen )
{
   return 0;
}
void CPROC ClientSetFontRendererData( Font font, POINTER pResult, _32 size )
{
}
...
PRELOAD( ClientDisplayImageRegisterInterface )
{
	...
	RegisterInterface( WIDE("image"), (POINTER(CPROC*)(void))_GetClientDisplayImageInterface, (void(CPROC*)(POINTER))_DropClientDisplayImageInterface );
}

IMAGE_NAMESPACE_END

// $Log: display_image_client.c,v $
// Revision 1.57  2005/06/24 15:11:46  jim
// Merge with branch DisplayPerformance, also a fix for watcom compilation
//
// Revision 1.56.2.2  2005/06/22 18:45:37  jim
// Fix final block transfer and decode (multiblock image transfer was 1 dword off on copying data into accumulation buffer.
//
// Revision 1.56.2.1  2005/06/22 17:31:43  jim
// Commit test optimzied display
//
// Revision 1.48  2005/06/04 00:05:59  d3x0r
// Protect against receiving events to closed displays post-mortum.
//
// Revision 1.47  2005/05/30 20:14:03  d3x0r
...
//
// Revision 1.39  2003/10/06 09:33:13  panther
...
//
// Revision 1.37  2003/09/21 16:25:28  panther
...
//
// Revision 1.36  2003/09/21 11:49:04  panther
// Fix service refernce counting for unload.  Fix Making sub images hidden.
// Fix Linking of services on server side....
// cleanup some comments...
//
// Revision 1.35  2003/09/19 16:40:35  panther
// Implement Adopt and Orphan sub image - for up coming Sheet Control
//
// Revision 1.34  2003/09/18 15:27:07  panther
// Fix export of IntersectRect, MergeRect interface
//
// Revision 1.33  2003/09/18 12:14:49  panther
// MergeRectangle Added.  Seems Control edit near done (fixing move/size errors)
//
// Revision 1.32  2003/09/15 17:06:37  panther
...
//
// Revision 1.31  2003/08/27 15:25:17  panther
