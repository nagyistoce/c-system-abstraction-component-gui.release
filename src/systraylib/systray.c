...
int APIENTRY IconMessageHandler( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
...
int RegisterIconEx( CTEXTSTR icon DBG_PASS )
{
	int status;
	int tried_default = 0;
	//NOTIFYICONDATA nid;
	if( !RegisterIconHandler( icon ) )
		return 0;
	if( !hMainMenu )
		BasicExitMenu();
