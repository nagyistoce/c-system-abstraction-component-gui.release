...
static void CPROC ReadClient( PCLIENT pc, POINTER buffer, int size, SOCKADDR *source )
{
	if( !buffer )
	{
...
static void CPROC ClientReceive( PCLIENT pc, POINTER buffer, int size, SOCKADDR *addr )
{
	if( buffer )
	{
