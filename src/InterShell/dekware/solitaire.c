   ...
   EasyRegisterResource( "InterShell/dekware/games/cards", CHECKBOX_VERTICAL, RADIO_BUTTON_NAME );
	...
	EasyRegisterResource( "InterShell/dekware/games/cards/select", CHECKBOX_SELECT_ALL              , RADIO_BUTTON_NAME );
	{
      ...
      FILE *file;
		...
		sprintf( filename, "images/cards/cardset1.config" );
...
OnSaveControl( "Games/Cards/Card Stack" )( FILE *file, PTRSZVAL psv )
{
	MyValidatedControlData( struct card_stack_control *, stack, (PSI_CONTROL)psv );
...
OnLoadControl( "Games/Cards/Card Stack" )( PCONFIG_HANDLER pch, PTRSZVAL psv )
{
...
static int OnMouseCommon( "Games/Cards/Card Stack" )(PSI_CONTROL pc, S_32 x, S_32 y, _32 b )
{
...
OnEditControl( "Games/Cards/Card Stack" )( PTRSZVAL psv, PSI_CONTROL parent )
{
	MyValidatedControlData( struct card_stack_control *, stack, (PSI_CONTROL)psv );
   PSI_CONTROL frame = LoadXMLFrameOver( parent, "ConfigureGameCardStack.isFrame" );
	...
	if( stack )
	{
      ...
      sprintf( buffer, "%d", stack->active.nMustPlay );
      SetControlText( GetControl( frame, EDIT_MUST_PLAY ), buffer );
      sprintf( buffer, "%d", stack->startup.nDrawAtStart );
      SetControlText( GetControl( frame, EDIT_DRAW_AT_START ), buffer );
      sprintf( buffer, "%d", stack->startup.nDrawAtDeal );
      SetControlText( GetControl( frame, EDIT_DRAW_AT_DEAL ), buffer );
      sprintf( buffer, "%d", stack->startup.nDrawDownAtStart );
...
OnCreateControl( "Games/Cards/Card Stack" )( PSI_CONTROL parent, S_32 x, S_32 y, _32 w, _32 h )
{
...
OnGetControl( "Games/Cards/Card Stack" )( PTRSZVAL psv )
{
   return (PSI_CONTROL)psv;
}

OnGlobalPropertyEdit( "Card Game" )( PSI_CONTROL parent_frame )
{
...
static void OnCloneControl( "Games/Cards/Card Stack" )( PTRSZVAL psvNew, PTRSZVAL psvOriginal )
{
...
OnKeyPressEvent( "Games/Cards/Start Game" )( PTRSZVAL psv )
{
...
OnCreateMenuButton( "Games/Cards/Start Game" )( PMENU_BUTTON button )
{
   return (PTRSZVAL)GetGame( "Stud" ); // default button
}

OnEditControl( "Games/Cards/Start Game" )( PTRSZVAL psv, PSI_CONTROL parent )
{
	//char buffer[256];
...
OnSaveControl( "Games/Cards/Start Game" )( FILE *file, PTRSZVAL psv )
{
	struct card_game *game = (struct card_game *)psv;
...
OnLoadControl( "Games/Cards/Start Game" )( PCONFIG_HANDLER pch, PTRSZVAL psv )
{
   //struct card_game *game = (struct card_game *)psv;
...
OnKeyPressEvent( "Games/Cards/Deal Game" )( PTRSZVAL psv )
{
	struct card_game *game = (struct card_game *)psv;
	//Shuffle( game->deck );
	{
...
OnCreateMenuButton( "Games/Cards/Deal Game" )( PMENU_BUTTON button )
{
   return (PTRSZVAL)GetGame( "Stud" ); // default button
}

OnEditControl( "Games/Cards/Deal Game" )( PTRSZVAL psv, PSI_CONTROL parent )
{
	//char buffer[256];
...
OnSaveControl( "Games/Cards/Deal Game" )( FILE *file, PTRSZVAL psv )
{
	struct card_game *game = (struct card_game *)psv;
   fprintf( file, "Start card game button game=\'%s\'\n", game->name );
}

OnLoadControl( "Games/Cards/Deal Game" )( PCONFIG_HANDLER pch, PTRSZVAL psv )
{
   //struct card_game *game = (struct card_game *)psv;
   AddConfigurationMethod( pch, "Start card game button game=\'%m\'", SetStartButtonGame );
}
