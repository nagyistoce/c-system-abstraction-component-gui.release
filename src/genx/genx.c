...
static utf8 storePrefix(genxWriter w, constUtf8 prefix, Boolean force)
{
  ...
  {
    sprintf((char *) buf, WIDE("xmlns:%s"), prefix);
    prefix = buf;
  }
				   ...
				   genxStatus * statusP)
{
  ...
  else
  {
    ...
    if (defaultPref == NULL)
    {
      sprintf((char *) newPrefix, WIDE("g%d"), w->nextPrefix++);
				      ...
				      genxStatus * statusP)
{
  ...
  else if (strncmp((const char *) name, WIDE("xmlns"), STRLEN_XMLNS_COLON - 1) == 0)
...
static genxStatus addAttribute(genxAttribute a, constUtf8 valuestr)
{
  ...
  if (valuestr)
  {
	...
	return w->status = GENX_NON_XML_CHARACTER;
      ...
      {
      case 9:
	collectPiece(w, &a->value, WIDE("&#x9;"), 5);
	break;
      case 0xa:
	collectPiece(w, &a->value, WIDE("&#xA;"), 5); 
	break;
      case 0xd:
	collectPiece(w, &a->value, WIDE("&#xD;"), 5); 
	break;
      case '"':
	collectPiece(w, &a->value, WIDE("&quot;"), 6);
	break;
      case '<':
	collectPiece(w, &a->value, WIDE("&lt;"), 4);
	break;
      case '&':
	collectPiece(w, &a->value, WIDE("&amp;"), 5);
	break;
...
genxStatus genxAddCharacter(genxWriter w, int _c)
{
	UTFTYPE cUTF8[10];
   unsigned int c = _c; // safer conversions with LARGE c's
  ...
  {
    *next++ = (UTFTYPE)0xc0 | (UTFTYPE)((c >> 6)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)((c & 0x3f)&0xFF);
  }
  else if (c < 0x10000)
  {
    *next++ = (UTFTYPE)0xe0 | (UTFTYPE)((c >> 12)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)(((c & 0xfc0) >> 6)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)((c & 0x3f)&0xFF);
  }
  else
  {
    *next++ = (UTFTYPE)0xf0 | (UTFTYPE)((c >> 18)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)(((c & 0x3f000) >> 12)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)(((c & 0xfc0) >> 6)&0xFF);
    *next++ = (UTFTYPE)0x80 | (UTFTYPE)((c & 0x3f)&0xFF);
  }
  *next = 0;
