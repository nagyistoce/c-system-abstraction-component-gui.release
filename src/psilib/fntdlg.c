...
typedef struct font_dialog_tag
{
	...
	Font pFont;   // temp font for drawing sample
	int done, okay;
	...
	void (CPROC* Update)(PTRSZVAL,Font);
	PTRSZVAL psvUpdate;
...
PRIORITY_PRELOAD( InitFontDialogGlobal, IMAGE_PRELOAD_PRIORITY + 1 )
{
   // static variable.
...
int UpdateSampleFont( PFONT_DIALOG pfd )
{
	...
	if( pfd->pFontEntry && pfd->pFontStyle && pfd->pSizeFile )
	{
		...
		pfd->pFont = InternalRenderFont( pfd->pFontEntry - fg.pFontCache
												 , pfd->pFontStyle - pfd->pFontEntry->styles
												 , pfd->pSizeFile - pfd->pFontStyle->files
												 , (short)(pfd->width_scale?ScaleValue(pfd->width_scale,pfd->nWidth):pfd->nWidth)
												 , (short)(pfd->height_scale?ScaleValue(pfd->height_scale,pfd->nHeight):pfd->nHeight)
   ...
   DisableUpdateListBox( pc, TRUE );
	...
	if( !pliSelect )
		pliSelect = GetNthItem( pc
									 , pfd->pFontEntry?(pfd->pFontEntry - fg.pFontCache):0 );
		...
		if( fdData.done || fdData.okay )
		{
			...
			if( fdData.okay )
			{
				if( pFontData )
				{
					...
					{
						_32 l1, l2, l3, l4;
						_32 resultsize = sizeof(FONTDATA)
						...
						pResult->nFamily = fdData.pFontEntry - fg.pFontCache;
						pResult->nStyle = fdData.pFontStyle - fdData.pFontEntry->styles;
						pResult->nFile = fdData.pSizeFile - fdData.pFontStyle->files;
...
void CPROC UpdateCommonFont( PTRSZVAL psvCommon, Font font )
{
