...
static int MenuMouse( PMENU pm, S_32 x, S_32 y, _32 b )
{
	...
	if( !pm )
	{
      lprintf( WIDE("mouse but not on my type?") );
		return FALSE;
	}
...
void UnshowMenu( PMENU pm )
{
   ...
   lprintf( WIDE("Popup release capture") );
	...
	if( pm->parent )
	{
		...
		pm->parent->selection = pm->selection;
...
void ShowMenu( PMENU pm, int x, int y, LOGICAL bWait, PSI_CONTROL parent )
{
	...
	if( ( x + pm->width + FrameBorderX(BORDER_NORMAL) ) >= cx )
	{
		if( pm->parent )
		{
		...
		{
         dx = cx - ( pm->width  + FrameBorderX(BORDER_NORMAL) );
		}
	}
	else
		dx = x;
   ...
   MemSet( pmi, 0, sizeof( MENUITEM ) );
	...
	else
	{
		if( pData )
		{
			...
			pmi->data.text.text = NewArray( TEXTCHAR, ( pmi->data.text.textlen = strlen( (TEXTSTR)pData ) ) + 1);
			strcpy( pmi->data.text.text, (CTEXTSTR)pData );
		}
		if( type & MF_POPUP )
		{
