...
//cpg27dec2006 console\WinLogic.c(19): Warning! W202: Symbol 'update_rect' has been defined, but not referenced
//cpg27dec2006 console\WinLogic.c(21): Warning! W202: Symbol '_mouse_buttons' has been defined, but not referenced
//cpg27dec2006 console\WinLogic.c(21): Warning! W202: Symbol 'mouse_buttons' has been defined, but not referenced
//cpg27dec2006 console\WinLogic.c(20): Warning! W202: Symbol 'mouse_x' has been defined, but not referenced
//cpg27dec2006 console\WinLogic.c(20): Warning! W202: Symbol 'mouse_y' has been defined, but not referenced
//cpg27dec2006 static PENDING_RECT update_rect;
//cpg27dec2006 static S_32 mouse_x, mouse_y;
//cpg27dec2006 static _32  mouse_buttons, _mouse_buttons;
...
/*
   ...
   if( pdp->common.Owner && pdp->common.Owner->pRecord )
   {
      ...
      nShow = snprintf( byName, sizeof( byName ), "(%s) "
...
void ConsoleCalculate( PCONSOLE_INFO pdp )
{
    //RECT rArea;
	int nLines;
...
#ifdef __DEKWARE_PLUGIN__
    if( pdp->common.Owner &&
         LocateMacro( pdp->common.Owner->Current, "WindowResize" ) )
    {
        char resize[256];
        sprintf( resize, "/WindowResize %d %d"
                            , pdp->nColumns
                            , pdp->nLines );
...
CORECON_EXPORT( volatile_variable_entry, vve_rows ) = { DEFTEXT( "rows" )
                                   , GetRows, NULL };
CORECON_EXPORT( volatile_variable_entry, vve_cols ) = { DEFTEXT( "cols" )
                                   , GetCols, NULL };
CORECON_EXPORT( volatile_variable_entry, vve_cursorx ) = { DEFTEXT( "cursorx" )
                                      , GetCursorX
                                      , SetCursorX };
CORECON_EXPORT( volatile_variable_entry, vve_cursory ) = { DEFTEXT( "cursory" )
                                      , GetCursorY
                                      , SetCursorY };
#endif
					  ...
					  )
{
   ...
   if( updated )
	{
			PENDING_RECT upd;
         //lprintf( "Okay well there's no data, first pass to check, end of stream... flush" );
         MemSet( &upd.cs, 0, sizeof( upd.cs ) );
			upd.flags.bTmpRect = 1;
         upd.flags.bHasContent = 0;
			...
			if( pmdp->Update && upd.flags.bHasContent )
			{
				...
				r.top = upd.y;
				r.bottom = upd.y + upd.height;
            //lprintf( "Doing update call to display..." );
				pmdp->Update( pmdp, &r );
			}
         updated = 0;
	}
...
TEXTCHAR *GetDataFromBlock( PCONSOLE_INFO pdp )
{
    ...
    TEXTCHAR *result = NewArray( TEXTCHAR, ( ( line_start - line_end ) + 1 ) * (pdp->nWidth + 2) );
    INDEX ofs = 0;
    int line, col;
    int first = TRUE;
    int _priorline;
...
CORECON_PROC( int, SetMode )( PDATAPATH xpdp, PSENTIENT ps, PTEXT params )
{
   ...
   while( ( temp = GetParam( ps, &params ) ) )
   {
      ...
      //if( pmdp->common.Type == myTypeID )
      {
         if( TextLike( temp, "direct" ) )
         {
            ...
            SetRegistryInt( "Dekware\\Wincon\\Direct"
                          ...
                          , pdp->flags.bDirect );
#endif
            ConsoleCalculate( pmdp );
            LeaveCriticalSec( &pdp->Lock );
         }
         else if( TextLike( temp, "line" ) )
         {
         ...
         else if( TextLike( temp, "buffer" ) )
         {
            pmdp->flags.bCharMode = FALSE;
         }
         else if( TextLike( temp, "char" ) )
         {
      ...
      //else
      //{
      //  DECLTEXT( msg, "Command path is not a PSI Console" );
      //  EnqueLink( &ps->Command->Output, &msg );
      //}
   }
   if( !firstopt )
   {
      DECLTEXT( msg, "Must specify mode: Direct, Line." );
      EnqueLink( &ps->Command->Output, &msg );
   }
   return 0;
}
...
#ifdef __DEKWARE_PLUGIN__
char *ColorTable[] = { "black", "blue", "green"
                     , "cyan", "red", "magenta"
                     , "brown", "grey", "darkgrey"
...
int ConSetColor( PDATAPATH pdp, PSENTIENT ps, PTEXT parameters )
{
   ...
   if( !color )
   {
      DECLTEXT( msg, "Parameters to SetColor are : Foreground Background" );
      EnqueLink( &ps->Command->Output, &msg );
      return 0;
   }
   for( i = 0; i < 16; i++ )
   ...
   {
      DECLTEXT( msg, "First parameter was not a known color..." );
      EnqueLink( &ps->Command->Output, &msg );
      return 0;
   }
   ...
   if( color )
   {
      ...
      {
         //DECLTEXT( msg, "Second parameter was not a known color..." );
         //EnqueLink( &ps->Command->Output, &msg );
    ...
    //                , pmdp->Display.History->pCursor->DefaultColor.background );
    //SetRegistryInt( "Dekware\\gggWincon\\Foreground"
    //                , GetText( GetName( pmdp->ps->Current ) )
...
int SetTabs( PDATAPATH pdp, PSENTIENT ps, PTEXT params )
{
    ...
    if( temp )
    {
        ...
        if( !pmdp )
        {
            DECLTEXT( msg, "Window datapath is not open" );
            EnqueLink( &ps->Command->Output, &msg );
            return 0;
        }
        else
		  {
            ...
            pmdp->pHistory->tabsize = IntNumber( temp );
        }
    }
    else
    {
        ...
        {
            DECLTEXT( msg, "Tab size not specified defaulting to 8." );
            EnqueLink( &ps->Command->Output, &msg );
