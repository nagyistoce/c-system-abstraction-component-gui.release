...
struct {
	CDATA color;
	CTEXTSTR name;
}color_defs[] = { { BASE_COLOR_BLACK, WIDE("black") }
...
void LoadButtonTheme( void )
{
	...
	if( !l.flags.theme_loaded )
	{
		...
		retval =  SACK_GetPrivateProfileStringEx("THEME","CURRENT"
	 											,"DEFAULT"
														,szTheme, sizeof(szTheme)
														  , gIniFileName, TRUE);
		snprintf( szBuffer, (sizeof (szBuffer) ), WIDE("THEME/%s"), szTheme);
		...
		{
			//lprintf( WIDE("setting an image file to null") );
	...
	else
	{
		...
		while( text && text[0] )
		{
			...
			}
			{
				...
				while( text && text[0] )
				{
               Font font = GetCommonFont( key->button );
					int len = strlen( text );
					_32 text_width, text_height;
					// handle content formatting.
					text++;
					GetStringSizeFontEx( text, len-1, &text_width, &text_height, font );
					//lprintf( WIDE("%d lines %d line %d %d"), lines, line, text_width, text_height );
					switch( text[-1] )
					{
					case 'A':
					case 'a':
		...
		for( layer = 0; ;layer++ )
		{
			if( layer == 0 )
			{
				...
				if( key->flags.background_image )
				{
					if( key->flags.bGreyed )
						BlotScaledImageSizedToMultiShadedAlpha( surface, key->background.image
																		  ...
																		  , Color( 172, 172, 172 )
																		  , Color( 192, 192, 192 )
																		  , Color( 172, 255, 172 )//Color( 162, 162, 162 )
																		  );
					else
...
#endif
				{
					...
					//if( IsControlFocused( key->button ) )
					//{
					//	BlatColorAlpha( surface, 0, 0, key->width, key->height
					//					  , AColor( 255, 255, 255, 48 ) );
					//}
...
void SetKeyText( PKEY_BUTTON key, CTEXTSTR newtext )
{
	...
	if( key )
	{
		...
		if( newtext )
		{
			...
			MemCpy( key->content, newtext, end - newtext + 1 );
		}
		else
			key->content = NULL;
		SmudgeCommon( (PCOMMON)key->button );
							 ...
							 )
{
   ...
   // make sure everything is zero... (specially with release mode)
								...
								);
	{
		...
		result->flags.bRidgeUnderText = FALSE;
...
void SetKeyTextField( PKEY_BUTTON pKey, PTEXT_PLACEMENT pField, CTEXTSTR text )
{
	if( pField )
	{
...
PTEXT_PLACEMENT AddKeyLayout( PKEY_BUTTON pKey, int x, int y, Font *font, CDATA color, _32 flags )
{
	PTEXT_PLACEMENT layout;
	//int count;
   //_32 w, h;
