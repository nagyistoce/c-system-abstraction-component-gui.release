...
PTRSZVAL Instance( GRAPH graph, GRAPH_LINE graph_line, TARGET_ADDRESS target )
{
	if( graph_line->type->parent )
	{
		...
		if( !line )
		{
			...
			{
            ThreadTo( FakeTimer, (PTRSZVAL)graph_line );
	...
	else if( graph_line->type->instance )
	{
		if( graph_line->psvInstance = graph_line->type->instance( target ) )
		{
            ...
            ThreadTo( FakeTimer2, (PTRSZVAL)graph_line );
				//graph_line->timer = AddTimer( 50, graph_line->type->timer, graph_line->psvInstance );
			}
		}
	}
...
void CPROC PingTimer( PTRSZVAL psv )
{
   ...
   //static char pResult[4096];
	...
	DoPingEx( (char*)pdc->address->name, 0, 150, 1, NULL/*pResult*/, FALSE, PingResult, (PTRSZVAL)pdc );
...
OnLoadCommon( "Ping Graph Registry" )( PCONFIG_HANDLER pch )
{
...
OnLoadControl( "Ping Status Graph" )( PCONFIG_HANDLER pch, PTRSZVAL psv )
{
...
OnSaveControl( "Ping Status Graph" )( FILE *file, PTRSZVAL psv )
{
	INDEX idx;
   GRAPH_LINE line;
	GRAPH graph = (GRAPH)psv;
...
OnLoadCommon( "Ping Status Graph Targets" )( PCONFIG_HANDLER pch )
{
   AddConfigurationMethod( pch, "ping target=%m", ReloadTarget );
}

OnSaveCommon( "Ping Status Graph Targets" )( FILE *file )
{
	INDEX idx;
   TARGET_ADDRESS target;
	LIST_FORALL( l.targets, idx, TARGET_ADDRESS, target )
...
static void CPROC CreateATarget( PTRSZVAL psv, PSI_CONTROL button )
{
	char buffer[256];
   TARGET_ADDRESS target;
	if( SimpleUserQuery( buffer, sizeof( buffer ), "Enter new target address", NULL ) )
	{
...
static void CPROC ButtonAddGraphLine( PTRSZVAL psv, PSI_CONTROL button )
{
	...
	}
	{
		...
		char value[64];
		snprintf( value, sizeof( value ), "%s of %s", graph_line->type->name, graph_line->target->name );
...
OnEditControl( "Ping Status Graph" )( PTRSZVAL psv, PSI_CONTROL parent )
{
	GRAPH graph = (GRAPH)psv;
	...
	if( frame )
	{
      ...
      SetListboxIsTree( list, TRUE );
		...
		if( list )
		{
         ...
         SetSelChangeHandler( list, SelectCurrentLine, (PTRSZVAL)graph );
			...
			{
				char value[64];
            snprintf( value, sizeof( value ), "%s of %s", line->type->name, line->target->name );
      ...
      EnableColorWellPick(
		...
		{
			char timespan[32];
			snprintf( timespan, sizeof( timespan ), "%ld.%03ld", graph->timespan / 1000, graph->timespan % 1000 );
		...
		}
		{
            ...
            pli = GetSelectedItem( GetControl( frame, LST_TARGET_NAMES ) );
				pli = GetSelectedItem( GetControl( frame, LST_GRAPH_TYPE ) );
				{
					...
					S_32 sec, secpart;
               int n;
					GetControlText( GetControl( frame, EDIT_GRAPH_TIMESPAN ), buffer, sizeof( buffer ) );
...
OnGetControl( "Ping Status Graph" )( PTRSZVAL psv )
{
	GRAPH graph = (GRAPH)psv;
   return graph->pc;
}
...
OnCreateControl( "Ping Status Graph" )( PSI_CONTROL parent, S_32 x, S_32 y, _32 w, _32 h )
{
...
OnDestroyControl( "Ping Status Graph" )( PTRSZVAL psv )
{
	GRAPH graph = (GRAPH)psv;
