...
static int rc_2pass1_create(xvid_plg_create_t * create, rc_2pass1_t ** handle)
{
	...
	if((rc->stat_file = fopen(param->filename, WIDE("w+b"))) == NULL)
		return(XVID_ERR_FAIL);
	 ...
	 * The File Header
	 */
	fprintf(rc->stat_file, WIDE("# XviD 2pass stat file (core version %d.%d.%d)\n"),
			XVID_VERSION_MAJOR(XVID_VERSION),
			XVID_VERSION_MINOR(XVID_VERSION),
			XVID_VERSION_PATCH(XVID_VERSION));
	fprintf(rc->stat_file, WIDE("# Please do not modify this file\n\n"));
...
static int rc_2pass1_destroy(rc_2pass1_t * rc, xvid_plg_destroy_t * destroy)
{
	if (rc->stat_file) {
		if (fclose(rc->stat_file) == EOF) {
			DPRINTF(XVID_DEBUG_RC, WIDE("Error closing stats file (%s)"), strerror(errno));
		}
	}
...
static int rc_2pass1_after(rc_2pass1_t * rc, xvid_plg_data_t * data)
{
	...
	fprintf(rc->stat_file, WIDE("%c %d %d %d %d %d %d\n"),
			type,
			stats->quant,
			stats->kblks,
			stats->mblks,
