						...
						int * intra_dc_threshold)
{
	...
	DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<video_packet_header>\n"));
	...
	DPRINTF(XVID_DEBUG_HEADER, WIDE("header_extension_code %i\n"), hec);
	if (hec)
	{
		int time_base;
		int time_increment;
	...
	if (dec->newpred_enable)
	{
		...
		DPRINTF(XVID_DEBUG_HEADER, WIDE("vop_id %i\n"), vop_id);
		if (BitstreamGetBit(bs))	/* vop_id_for_prediction_indication */
		{
			...
			DPRINTF(XVID_DEBUG_HEADER, WIDE("vop_id_for_prediction %i\n"), vop_id_for_prediction);
		}
		READ_MARKER();
	}
					 ...
					 WARPPOINTS *gmc_warp)
{
	...
	do {
		...
		if (start_code == VISOBJSEQ_START_CODE) {
			...
			DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<visual_object_sequence>\n"));

			BitstreamSkip(bs, 32);	/* visual_object_sequence_start_code */
			profile = BitstreamGetBits(bs, 8);	/* profile_and_level_indication */

			DPRINTF(XVID_DEBUG_HEADER, WIDE("profile_and_level_indication %i\n"), profile);
			...
			DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<visual_object>\n"));
			...
			{
				DPRINTF(XVID_DEBUG_ERROR, WIDE("visual_object_type != video\n"));
				return -1;
			}
			BitstreamSkip(bs, 4);
			...
			{
				DPRINTF(XVID_DEBUG_HEADER, WIDE("+ is_object_layer_identifier\n"));
			...
			{
				DPRINTF(XVID_DEBUG_HEADER, WIDE("+ aspect_ratio_info\n"));
			...
			{
				DPRINTF(XVID_DEBUG_HEADER, WIDE("+ vol_control_parameters\n"));
				BitstreamSkip(bs, 2);	/* chroma_format */
				dec->low_delay = BitstreamGetBit(bs);	/* low_delay */
				DPRINTF(XVID_DEBUG_HEADER, WIDE("low_delay %i\n"), dec->low_delay);
			...
			DPRINTF(XVID_DEBUG_HEADER, WIDE("shape %i\n"), dec->shape);
			if (dec->shape != VIDOBJLAY_SHAPE_RECTANGULAR)
			{
			...
			{
				DPRINTF(XVID_DEBUG_HEADER, WIDE("+ fixed_vop_rate\n"));
			...
			if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
				...
				if (dec->shape == VIDOBJLAY_SHAPE_RECTANGULAR) {
					...
					DPRINTF(XVID_DEBUG_HEADER, WIDE("width %i\n"), width);
					DPRINTF(XVID_DEBUG_HEADER, WIDE("height %i\n"), height);

					if (dec->width != width || dec->height != height)
					{
						if (dec->fixed_dimensions)
						{
							DPRINTF(XVID_DEBUG_ERROR, WIDE("decoder width/height does not match bitstream\n"));
							return -1;
						}
						resize = 1;
						dec->width = width;
				...
				DPRINTF(XVID_DEBUG_HEADER, WIDE("interlacing %i\n"), dec->interlacing);

				if (!BitstreamGetBit(bs))	/* obmc_disable */
				{
					DPRINTF(XVID_DEBUG_ERROR, WIDE("obmc_disabled==false not supported\n"));
					/* TODO */
				...
				{
					DPRINTF(XVID_DEBUG_HEADER, WIDE("not_8_bit==true (ignored)\n"));
				...
				DPRINTF(XVID_DEBUG_HEADER, WIDE("quant_type %i\n"), dec->quant_type);
				...
				if (dec->quant_type) {
					if (BitstreamGetBit(bs))	/* load_intra_quant_mat */
					{
						...
						DPRINTF(XVID_DEBUG_HEADER, WIDE("load_intra_quant_mat\n"));
					...
					if (BitstreamGetBit(bs))	/* load_inter_quant_mat */
					{
						...
						DPRINTF(XVID_DEBUG_HEADER, WIDE("load_inter_quant_mat\n"));
					...
					if (dec->shape == VIDOBJLAY_SHAPE_GRAYSCALE) {
						DPRINTF(XVID_DEBUG_ERROR, WIDE("greyscale matrix not supported\n"));
						return -1;
					}
				...
				{
					DPRINTF(XVID_DEBUG_ERROR, WIDE("data_partitioned not supported\n"));
				...
				if (vol_ver_id != 1) {
					...
					{
						DPRINTF(XVID_DEBUG_HEADER, WIDE("+ newpred_enable\n"));
					...
					DPRINTF(XVID_DEBUG_HEADER, WIDE("reduced_resolution_enable %i\n"), dec->reduced_resolution_enable);
				}
				else
				{
				...
				if (dec->scalability)
				{
					DPRINTF(XVID_DEBUG_ERROR, WIDE("scalability not supported\n"));
			...
			DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<group_of_vop>\n"));
			...
			BitstreamSkip(bs, 32);
			{
				...
				DPRINTF(XVID_DEBUG_HEADER, WIDE("time %ih%im%is\n"), hours,minutes,seconds);
			}
			...
			DPRINTF(XVID_DEBUG_HEADER, WIDE("time_base %i\n"), time_incr);
			DPRINTF(XVID_DEBUG_HEADER, WIDE("time_increment %i\n"), time_increment);

			DPRINTF(XVID_DEBUG_TIMECODE, WIDE("%c %i:%i\n"),
			...
			{
				DPRINTF(XVID_DEBUG_HEADER, WIDE("vop_coded==false\n"));
				return N_VOP;
			}
			...
			if ((dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) &&
				...
				*rounding = BitstreamGetBit(bs);	/* rounding_type */
				DPRINTF(XVID_DEBUG_HEADER, WIDE("rounding %i\n"), *rounding);
			}
			...
			if (dec->reduced_resolution_enable &&
				...
				*reduced_resolution = BitstreamGetBit(bs);
				DPRINTF(XVID_DEBUG_HEADER, WIDE("reduced_resolution %i\n"), *reduced_resolution);
			}
			else
			{
			...
			if (dec->shape != VIDOBJLAY_SHAPE_RECTANGULAR) {
				if(!(dec->sprite_enable == SPRITE_STATIC && coding_type == I_VOP)) {
					...
					DPRINTF(XVID_DEBUG_HEADER, WIDE("width %i\n"), width);
					DPRINTF(XVID_DEBUG_HEADER, WIDE("height %i\n"), height);
					DPRINTF(XVID_DEBUG_HEADER, WIDE("horiz_mc_ref %i\n"), horiz_mc_ref);
					DPRINTF(XVID_DEBUG_HEADER, WIDE("vert_mc_ref %i\n"), vert_mc_ref);
				}
			...
			if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
				...
				*intra_dc_threshold =
				...
				if (dec->interlacing) {
					...
					DPRINTF(XVID_DEBUG_HEADER, WIDE("interlace top_field_first %i\n"), dec->top_field_first);
					dec->alternate_vertical_scan = BitstreamGetBit(bs);
					DPRINTF(XVID_DEBUG_HEADER, WIDE("interlace alternate_vertical_scan %i\n"), dec->alternate_vertical_scan);
			...
			if (coding_type != I_VOP) {
				*fcode_forward = BitstreamGetBits(bs, 3);	/* fcode_forward */
				DPRINTF(XVID_DEBUG_HEADER, WIDE("fcode_forward %i\n"), *fcode_forward);
			}

			if (coding_type == B_VOP) {
				*fcode_backward = BitstreamGetBits(bs, 3);	/* fcode_backward */
				DPRINTF(XVID_DEBUG_HEADER, WIDE("fcode_backward %i\n"), *fcode_backward);
			}
			if (!dec->scalability) {
				if ((dec->shape != VIDOBJLAY_SHAPE_RECTANGULAR) &&
					(coding_type != I_VOP)) {
			...
			DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<user_data>: %s\n"), tmp);

			/* read xvid bitstream version */
			if(strncmp(tmp, WIDE("XviD"), 4) == 0) {
				if (tmp[strlen(tmp)-1] == 'C') {				
					sscanf(tmp, WIDE("XviD%dC"), &dec->bs_version);
					dec->cartoon_mode = 1;
				}
				else
					sscanf(tmp, WIDE("XviD%d"), &dec->bs_version);

				DPRINTF(XVID_DEBUG_HEADER, WIDE("xvid bitstream version=%i\n"), dec->bs_version);
			}

		    /* divx detection */
			i = sscanf(tmp, WIDE("DivX%dBuild%d%c"), &version, &build, &packed);
			if (i < 2)
				i = sscanf(tmp, WIDE("DivX%db%d%c"), &version, &build, &packed);

			if (i >= 2)
			{
				dec->packed_mode = (i == 3 && packed == 'p');
				DPRINTF(XVID_DEBUG_HEADER, WIDE("divx version=%i, build=%i packed=%i\n"),
		...
		} else					/* start_code == ? */
		{
			if (BitstreamShowBits(bs, 24) == 0x000001) {
				DPRINTF(XVID_DEBUG_STARTCODE, WIDE("<unknown: %x>\n"), BitstreamShowBits(bs, 32));
			}
			BitstreamSkip(bs, 8);
		}
	}
