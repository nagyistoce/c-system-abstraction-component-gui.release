...
/* decoding */
typedef void CPROC (COPY_UPSAMPLED_8X8_16TO8) (uint8_t *Dst, const int16_t *Src, const int BpS);
typedef void CPROC (ADD_UPSAMPLED_8X8_16TO8) (uint8_t *Dst, const int16_t *Src, const int BpS);

/* deblocking: Note: "Nb"_Blks is the number of 8-pixels blocks to process */
typedef void CPROC HFILTER_31(uint8_t *Src1, uint8_t *Src2, int Nb_Blks);
typedef void CPROC VFILTER_31(uint8_t *Src1, uint8_t *Src2, const int BpS, int Nb_Blks);

/* encoding: WARNING! These read 1 pixel outside of the input 16x16 block! */
typedef void CPROC FILTER_18X18_TO_8X8(int16_t *Dst, const uint8_t *Src, const int BpS);
typedef void CPROC FILTER_DIFF_18X18_TO_8X8(int16_t *Dst, const uint8_t *Src, const int BpS);
