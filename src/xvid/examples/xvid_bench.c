...
void test_dct()
{
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		printf( WIDE("%s -  %.3f usec       PSNR=%.3f  MSE=%.3f %s\n"),
				cpu->name, t, PSNR, MSE,
				(ABS(MSE)>=64)? "| ERROR" :"");
	}
}
...
void test_sad()
{
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		emms();
		t = (gettime_usec() - t) / nb_tests;
		printf( WIDE("%s - sad16bi %.3f usec       sad=%d %s\n"),
				cpu->name, t, s,
				(s!=26274)?"| ERROR": "" );
		...
		emms();
		t = (gettime_usec() - t) / nb_tests;
		printf( WIDE("%s - dev16   %.3f usec       sad=%d %s\n"),
				cpu->name, t, s,
				(s!=3344)?"| ERROR": "" );
...
void test_transfer()
{
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		printf( WIDE("%s - 16to8     %.3f usec       crc32=0x%08x %s\n"),
				cpu->name, t, s,
				(s!=0xee7ccbb4)?"| ERROR": "");
...
void report_stats(STATS_8x8 *S, const double *Limits)
{
	...
	printf( WIDE("Peak error:   %4.4f\n"), PE );
	printf( WIDE("Peak MSE:     %4.4f\n"), PMSE );
	printf( WIDE("Overall MSE:  %4.4f\n"), OMSE );
	printf( WIDE("Peak ME:      %4.4f\n"), PME );
	printf( WIDE("Overall ME:   %4.4f\n"), OME );

	if (Limits!=0)
	{
		printf( WIDE("[PE<=%.4f %s]  "), Limits[0], CHECK(PE,   Limits[0]) );
		printf( WIDE("\n") );
		printf( WIDE("[PMSE<=%.4f %s]"), Limits[1], CHECK(PMSE, Limits[1]) );
		printf( WIDE("[OMSE<=%.4f %s]"), Limits[2], CHECK(OMSE, Limits[2]) );
		printf( WIDE("\n") );
		printf( WIDE("[PME<=%.4f %s] "), Limits[3], CHECK(PME , Limits[3]) );
		printf( WIDE("[OME<=%.4f %s] "), Limits[4], CHECK(OME , Limits[4]) );
		printf( WIDE("\n") );
	}
}
...
void test_dec(const char *name, int width, int height, int with_chksum)
{
	...
	f = fopen(name, WIDE("rb"));
	if (f==0) {
		printf( WIDE("can't open file '%s'\n"), name);
		return;
	}
	fseek(f, 0, SEEK_END);
	buf_size = ftell(f);
	...
	t += gettime_usec();
	if (t>0.)
		printf( WIDE("%d frames decoded in %.3f s -> %.1f FPS\n"), nb, t*1.e-6f, (float)(nb*1.e6f/t) );
	if (with_chksum)
...
void test_bugs1()
{
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		printf( WIDE("dequant_mpeg_intra with CPU=%s:  "), cpu->name);
		printf( WIDE("  Out[]= ") );
		for(i=0; i<64; ++i) printf( WIDE("[%d]"), Dst[i]);
		printf( WIDE("\n") );
	}
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		printf( WIDE("dequant_mpeg_inter with CPU=%s:  "), cpu->name);
		printf( WIDE("  Out[]= ") );
		for(i=0; i<64; ++i) printf( WIDE("[%d]"), Dst[i]);
		printf( WIDE("\n") ); 
	}
...
void test_dct_precision_diffs()
{
	...
	for(cpu = cpu_list; cpu->name!=0; ++cpu)
	{
		...
		fdct(Blk);
		idct(Blk);
		printf( WIDE(" fdct+idct diffs with CPU=%s: \n"), cpu->name );
		for(i=0; i<8; ++i) {
			int j;
			for(j=0; j<8; ++j) printf( WIDE(" %d "), Blk[i*8+j]-Blk0[i*8+j]); 
			printf("\n"); 
		}
		printf("\n"); 
	}
		...
		for(q=1; q<=max_Q; ++q)
			for(i=0; i<n-1; ++i)
				if (Crcs_Inter[i][q]!=Crcs_Inter[i+1][q])
					printf( WIDE("Discrepancy Inter: qm=%d, q=%d  -> %d/%d !\n"),
		...
		for(q=1; q<=max_Q; ++q)
			for(i=0; i<n-1; ++i)
				if (Crcs_Intra[i][q]!=Crcs_Intra[i+1][q])
					printf( WIDE("Discrepancy Intra: qm=%d, q=%d  -> %d/%d!\n"),
							qm, q, Crcs_Inter[i][q], Crcs_Inter[i+1][q]);
	}
#endif
}
