 ...
 ****************************************************************************/
	...
	for (i=1; i< argc; i++) {
		...
		if (strcmp("-asm", argv[i]) == 0 ) {
			...
			i++;
			if (strcmp(argv[i], WIDE("rgb16")) == 0) {
				CSP = XVID_CSP_RGB555;
				BPP = 2;
			} else if (strcmp(argv[i], WIDE("rgb24")) == 0) {
				CSP = XVID_CSP_BGR;
				BPP = 3;
			} else if (strcmp(argv[i], WIDE("rgb32")) == 0) {
				CSP = XVID_CSP_BGRA;
				BPP = 4;
			} else if (strcmp(argv[i], WIDE("yv12")) == 0) {
				CSP = XVID_CSP_YV12;
				BPP = 1;
			} else {
				CSP = XVID_CSP_I420;
	...
	if (ARG_INPUTFILE==NULL) {
		fprintf(stderr, WIDE("Warning: MSVC build does not read EOF correctly from stdin. Use the -i switch.\n\n"));
	}
#endif
 ...
 ****************************************************************************/
	...
	do {
		...
		do {
			...
			if(xvid_dec_stats.type == XVID_TYPE_VOL) {
				...
				if(XDIM*YDIM < xvid_dec_stats.data.vol.width*xvid_dec_stats.data.vol.height) {
					...
					fprintf(stderr, WIDE("Resized frame buffer to %dx%d\n"), XDIM, YDIM);
				}
			}
		...
		if(ARG_SAVEMPEGSTREAM) {
			...
			sprintf(filename, WIDE("%sframe%05d.m4v"), filepath, filenr);
			filehandle = fopen(filename, WIDE("wb"));
			if(!filehandle) {
				fprintf(stderr,
		...
		if (ARG_SAVEDECOUTPUT) {
			sprintf(filename, WIDE("%sdec%05d"), filepath, filenr);
 ...
 ****************************************************************************/
 ...
 release_all:
  	if (dec_handle) {
	  	status = dec_stop();
		if (status)    
			fprintf(stderr, WIDE("decore RELEASE problem return value %d\n"), status);
	}
...
static void usage()
{
	...
	fprintf(stderr, WIDE("Usage : xvid_decraw [OPTIONS]\n"));
	fprintf(stderr, WIDE("Options :\n"));
	fprintf(stderr, WIDE(" -asm           : use assembly optimizations (default=disabled)\n"));
	fprintf(stderr, WIDE(" -i string      : input filename (default=stdin)\n"));
	fprintf(stderr, WIDE(" -d             : save decoder output\n"));
	fprintf(stderr, WIDE(" -c csp         : choose colorspace output (rgb16, rgb24, rgb32, yv12, i420)\n"));
	fprintf(stderr, WIDE(" -f format      : choose output file format (tga, pnm, pgm)\n"));
	fprintf(stderr, WIDE(" -m             : save mpeg4 raw stream to individual files\n"));
	fprintf(stderr, WIDE(" -help          : This help message\n"));
	fprintf(stderr, WIDE(" (* means default)\n"));
...
static int write_image(char *prefix, unsigned char *image)
{
	...
	if (FORMAT == USE_PNM && BPP == 1) {
		...
		fprintf(stderr, WIDE("Bug: should not reach this path code -- please report to xvid-devel@xvid.org with command line options used"));
		exit(-1);
	}

	sprintf(filename, WIDE("%s.%s"), prefix, ext);
