...
void config_reg_get(CONFIG * config)
{
	...
	if (m_hdll != NULL) {
		...
		((int (__cdecl *)(void *, int, void *, void *))GetProcAddress(m_hdll, WIDE("xvid_global")))
...
{
	char buf[FLOAT_BUF_SZ];
	sprintf(buf, WIDE("%.2f"), (float)value/100);
...
static void adv_mode(HWND hDlg, int idd, CONFIG * config)
{
	...
	case IDD_BITRATE :
		{
			...
			if (vsize > 0) {
				...
				SetDlgItemText(hDlg, IDC_BITRATE_VSIZE, WIDE("Overflow"));
				SetDlgItemText(hDlg, IDC_BITRATE_VRATE, WIDE("Overflow"));
			}
	...
	case IDD_ZONE :
		...
		if (weight_en) {
			...
			SetDlgItemText(hDlg, IDC_ZONE_MIN, WIDE("0.01"));
			SetDlgItemText(hDlg, IDC_ZONE_MAX, WIDE("2.00"));
		}else if (quant_en) {
			...
			SetDlgItemText(hDlg, IDC_ZONE_MIN, WIDE("1"));
			SetDlgItemText(hDlg, IDC_ZONE_MAX, WIDE("31"));
		}
			...
			case IDC_BITRATE_ASELECT :
				...
				if (GetOpenFileName(&ofn)) {
					...
					if ((hFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0)) == INVALID_HANDLE_VALUE ||
						...
						MessageBox(hDlg, WIDE("Could not get file size"), WIDE("Error"), 0);
					}else{
						SetDlgItemInt(hDlg,
...
static void main_mode(HWND hDlg, CONFIG * config)
{
	...
	if (g_use_bitrate) {
		SetDlgItemText(hDlg, IDC_BITRATE_S, WIDE("Target bitrate (kbps):"));

		wsprintf(buf, WIDE("%i kbps"), DEFAULT_MIN_KBPS);
		SetDlgItemText(hDlg, IDC_BITRATE_MIN, buf);

		max = profiles[profile].max_bitrate;
		if (max == 0) max = DEFAULT_MAX_KBPS;
		wsprintf(buf, WIDE("%i kbps"), max);
		...
		SetDlgItemText(hDlg, IDC_BITRATE_S, WIDE("Target size (kbytes):"));
	} else if (rc_mode==RC_MODE_1PASS) {
		SetDlgItemText(hDlg, IDC_BITRATE_S, WIDE("Target quantizer:"));
  		SendDlgItemMessage(hDlg, IDC_SLIDER, TBM_SETRANGE, TRUE, MAKELONG(100, 3100));
		SendDlgItemMessage(hDlg, IDC_SLIDER, TBM_SETPOS, TRUE,
							get_dlgitem_float(hDlg, IDC_BITRATE, DEFAULT_QUANT ));
		SetDlgItemText(hDlg, IDC_BITRATE_MIN, WIDE("1 (maximum quality)"));
		SetDlgItemText(hDlg, IDC_BITRATE_MAX, WIDE("(smallest file) 31"));
	...
	case WM_COMMAND :
		if (HIWORD(wParam) == BN_CLICKED) {
			...
			case IDC_ADD :
			{
				...
				if (config->num_zones >= MAX_ZONES) {
					MessageBox(hDlg, WIDE("Exceeded maximum number of zones.\nIncrease config.h:MAX_ZONES and rebuild."), WIDE("Warning"), 0);
					break;
				}
			...
			case IDC_REMOVE :
			{
				...
				if (sel == -1 || config->num_zones < 1) {
					/*MessageBox(hDlg, WIDE("Nothing selected"), WIDE("Warning"), 0);*/
					break;
				}
...
BOOL CALLBACK about_proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG :
		{
			...
			if (m_hdll != NULL) {
				...
				wsprintf(core, WIDE("xvidcore.dll version %d.%d.%d (\")%s\")",
				...
				wsprintf(core, WIDE("xvidcore.dll not found!"));
			}
	...
	case WM_COMMAND :
		...
		{
			ShellExecute(hDlg, WIDE("open"), XVID_WEBSITE, NULL, NULL, SW_SHOWNORMAL);
		}
