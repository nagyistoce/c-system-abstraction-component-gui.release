...
typedef struct com_tracking_tag {
	...
	int     iUsers;
	struct {
		unsigned int bUseCarrierDetect : 1;
		unsigned int bHaveCarrier : 1;
		unsigned int bInRead : 1;
		unsigned int bOutputOnly : 1;
		unsigned int bDestroy : 1;
		unsigned int bInTimer : 1;
   } flags;
...
//-----------------------------------------------------------------------
int OpenComm( const char *name, int nInQueue, int nOutQueue )
{
	gbLog = SACK_GetPrivateProfileIntEx( WIDE("futlmsg"), WIDE("allowlogging"), 0, WIDE("ftn_soft.ini"), TRUE );
	if( gbLog )
		Log1( WIDE("Going to open:%s"), name );
	{
		...
		HANDLE hCom = CreateFile( name, GENERIC_READ|GENERIC_WRITE
									  ...
									  , NULL );
		timeout.ReadIntervalTimeout = SACK_GetPrivateProfileInt( name, "port timeout", 100, "ftn_soft.ini" );
...
PCOM_TRACK FindComByName( LPCSTR szPort )
{
...
PCOM_TRACK AddComTracking( LPCSTR szPort, int iCommId )
{
...
static int 
    ParseComString ( const char far *p
                   , _32 far *pBaud
                   , int far *pPar
                   , int far *pData
                   , int far *pStop
                   ...
                   , int *piRTS
                   , int *piRTSFlow
                   , const char far * far *ppErr
                   )
{
  int iPar = 0, iData, iStop;
  _32 dwBaud;
//#if defined( _WIN32 ) || defined( BCC16 )
  dwBaud = (unsigned long)strtoul ( p, (char far * far *)&p, 10 );
  if ( *p != ',' )
  {
    if ( ppErr )
      *ppErr = "{baud}?";
													 ...
													 , int readlen )
{
	...
	lprintf( "updating read length to %d %p %d", iCommId, pct, pct->nReadTotal );
	if( pct )
	{
    ...
    * If we have exceeded the timeout limit, then set the error.      *
    \*****************************************************************/  
    	if( pComTrack->current.nTotalRead )
    	{
	      ...
	      Log1( DBG_FILELINEFMT "SackCommReadBuffer() => TIMEOUT (WITH DATA %d)"
	             DBG_RELAY, pComTrack->current.nTotalRead );
