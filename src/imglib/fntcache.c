...
_32 IndexOf( TEXTCHAR **list, _32 count, POINTER item )
{
	INDEX idx;
			...
			default:
				{
					...
					*(count++) = 0;
					...
					pfe->name = (PDICT_ENTRY)build.pFamilyList[atol( family )];
			...
			case '!':
				...
				while( ( style = next ) && next[0] )
				{
					...
					*(flags++) = 0;
					...
					pfs->name = (PDICT_ENTRY)build.pStyleList[atol( style )];

					pfs->nFiles = 0;
					pfs->appfiles = build.pSizeFileSlab + build.nSizeFile;
					build.nSizeFile += atol( count );
					//Allocate( sizeof( APP_SIZE_FILE ) * atol( count ) );
						...
						while( ( count = next ) && next[0] )
						{
							if( count[0] )
							{
								...
								*(PathID++) = 0;
								...
								psfCurrent->path =build.pPathList[atol( PathID )];
								psfCurrent->file =build.pFileList[atol(file)];
								psfCurrent->nSizes = 0;
								psfCurrent->sizes = build.pSizeSlab + build.nSize;
								build.nSize += atol( count2 );
									//Allocate( sizeof( SIZES ) * atol( count2 ) );
								psfCurrent->nAlternate = 0;
								psfCurrent->pAlternate = build.pAltSlab + build.nAlt;
								build.nAlt += atol( count );
							}
							//Allocate( sizeof( ALT_SIZE_FILE ) * atol(count) );
			...
			case '|':
							...
							while( ( PathID = next ) && PathID[0] != ':' )
							{
								...
								*(file++) = 0;
								if( next )
									*(next++) = 0;
								pasf->path =(PDICT_ENTRY)build.pPathList[atol(PathID)];
								pasf->file =(PDICT_ENTRY)build.pFileList[atol(file)];
							}
							if( PathID )
							{
								// +1 == ':' +2 == ... char type...
