...
#if defined( USE_SQLITE ) || defined( USE_SQLITE_INTERFACE )
void GetNowFunc(sqlite3_context*onwhat,int n,sqlite3_value**something)
{
...
void SQLiteGetLastInsertID(sqlite3_context*onwhat,int n,sqlite3_value**something)
{
	...
	snprintf( str, sizeof( str ), "%Ld", sqlite3_last_insert_rowid( odbc->db ) );
	printf( "x = %s", str );
	sqlite3_result_text( onwhat, str, (int)(sizeof( str[0] ) * StrLen( str )), 0 );
}
...
int SimpleSize( POINTER p )
{
   return SizeOfMemBlock( p );
}
...
#if defined( USE_SQLITE ) || defined( USE_SQLITE_INTERFACE )
		, odbc->flags.bSQLite_native?"[Native SQLite]":"[Not Native SQLite]"
#else
		, "[]"
#endif
#ifdef USE_ODBC
		, odbc->flags.bODBC?"[ODBC]":"[Not ODBC]"
#else
		, "[]"
#endif
		);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
PTRSZVAL CPROC SetPrimaryDSN( PTRSZVAL psv, arg_list args )
{
	PARAM( args, CTEXTSTR, pDSN );
	strcpy( g.Primary.info.pDSN, pDSN );
	return psv;
}

PTRSZVAL CPROC SetOptionDSN( PTRSZVAL psv, arg_list args )
{
	PARAM( args, CTEXTSTR, pDSN );
	strcpy( g.Option.info.pDSN, pDSN );
	return psv;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
PTRSZVAL CPROC SetBackupDSN( PTRSZVAL psv, arg_list args )
{
	PARAM( args, CTEXTSTR, pDSN );
	strcpy( g.Backup.info.pDSN, pDSN );
	return psv;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
PTRSZVAL CPROC SetUser( PTRSZVAL psv, arg_list args )
{
	PARAM( args, CTEXTSTR, pUser );
	strcpy( g.Primary.info.pID, pUser );
	return psv;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
PTRSZVAL CPROC SetFallback( PTRSZVAL psv, arg_list args )
{
...
PTRSZVAL CPROC SetLoggingEnabled( PTRSZVAL psv, arg_list args )
{
	PARAM( args, LOGICAL, bEnable );
	g.flags.bLogging = bEnable;
	return psv;
}
PTRSZVAL CPROC SetLoggingEnabled2( PTRSZVAL psv, arg_list args )
{
	PARAM( args, LOGICAL, bEnable );
...
void InitLibrary( void )
{
	...
	if( !g.flags.bInited )
	{
      ...
      strcpy( g.Option.info.pDSN, "option.db" );
		{
...
//PRIORITY_PRELOAD( ZInitSQLLibrary, SQL_PRELOAD_PRIORITY )
//{
//   InitLibrary();
//}
	...
	{
		{
			...
			{
				TEXTCHAR msg[80];
				snprintf( msg, sizeof( msg ), "Connecting to [%s]%*.*s", odbc->info.pDSN, 3+state, 3+state,"........." );
...
#error sorry, need to convert from unicode native to unsigned char * type thing...
			...
			( g.flags.bFallback || odbc->flags.bSkipODBC ) )
		{
         ...
         // and - we REQUIRE connection...
			...
			{
				rc3 = sqlite3_open( odbc->info.pDSN, &odbc->db );
				if( rc3 )
				{
      ...
      TEXTSTR outdata;
		...
		if( collection->flags.bBuildResultArray )
		{
			...
			if( collection->columns )
			{
				...
				SendMultiServiceEventPairs( collection->SourceID, responce + g.SQLMsgBase, collection->columns * 2 + 1, pairs );
			}
			else
			{
				SendMultiServiceEventPairs( collection->SourceID, responce + g.SQLMsgBase, 0, NULL );
			}
		}
		else
		{
			...
			while( len > 4096 )
			{
				...
				if( first )
					SendServiceEvent( collection->SourceID, WM_SQL_DATA_START + g.SQLMsgBase, outdata, 4096 );
				//PostMessage( collection->hWnd, WM_SQL_DATA_START, (WPARAM)hWnd, (LPARAM)outatom );
				else
					SendServiceEvent( collection->SourceID, WM_SQL_DATA_MORE + g.SQLMsgBase, outdata, 4096 );
			...
			SendServiceEvent( collection->SourceID, responce + g.SQLMsgBase, outdata, len );
...
PCOLLECT FindCollection( PODBC odbc, _32 SourceID )
{
...
PCOLLECT Collect( PCOLLECT collection, _32 *params, _32 paramlen )
{
		...
		else
		{
            ...
            */
            ...
            //  SQLITE_CONSTRAINT - statement like an insert with a key that already exists.
				lprintf( "Unknown, unhandled SQLITE error: %s", sqlite3_errmsg(odbc->db ) );
				//DebugBreak();
				break;
			}
					...
					for( idx = 1; idx <= collection->columns; idx++ )
					{
						...
						if( odbc->flags.bSQLite_native )
						{
							collection->fields[idx-1] =
								(char*)sqlite3_column_name(collection->stmt
																  , idx - 1 );
						}
#endif
#if ( defined( USE_SQLITE ) || defined( USE_SQLITE_INTERFACE ) ) && defined( USE_ODBC )
...
#error sorry, need to convert from unicode native to unsigned char * type thing...
				...
				if( odbc->flags.bSQLite_native )
				{
					...
					if( collection->flags.bBuildResultArray )
					{
						...
						case SQLITE_BLOB:
							...
							break;
						default:
							collection->results[idx-1] = StrDup( text );
							break;
						}
					}
					else
					{
						vtprintf( collection->pvt_result, WIDE("%s%s"), first?"":",", text?text:"" );
						first=0;
					}
				}
#endif
					...
					rc == SQL_SUCCESS_WITH_INFO )
				{
					...
					if( ResultLen > 0 )
					{
						...
						else
						{
							...
							*/
							...
							{
							POINTER tmp;
							vtprintf( collection->pvt_result, WIDE("%s%s"), first?"":",", tmp = Deblobify( byResult, collection->colsizes[idx-1] ) );
							Release( tmp );
							}
							else
							*/
							vtprintf( collection->pvt_result, WIDE("%s%s"), first?"":",", byResult );
							first = 0;
						}
					}
					else
					{
						if( !collection->flags.bBuildResultArray )
						{
							...
							vtprintf( collection->pvt_result, WIDE("%s"), first?"":"," );
							first=0;
						}
	...
	GenerateResponce( collection, result_cmd );
	//if( result_cmd == WM_SQL_RESULT_ERROR )
	//	DestroyCollector( collection );
	return 0;
}
...
//		{
  //       lprintf( "Releasing old handle... %p", collection->hstmt );
	...
	if( !query ) 
		query = GetText( VarTextPeek( collection->pvt_out ) );
	else
	{
		VarTextEmpty( collection->pvt_out );
	...
	if( !query )
	{
		xlprintf(LOG_ALWAYS)( WIDE("REally should pass a query if you expect a query.") );
		DebugBreak();
		return FALSE;
	}
	if( EnsureLogOpen(odbc ) )
   ...
   if( odbc->flags.bSQLite_native )
	{
		const char *tail;
		// can get back what was not used when parsing...
		rc3 = sqlite3_prepare_v2( odbc->db, query, -1, &collection->stmt, &tail );
		if( rc3 )
		{
			const char *tmp;
			//DebugBreak();
...
void PopODBCExx( PODBC odbc, LOGICAL bAllowNonPush )
{
	if( !odbc )
		odbc = g.odbc;
	if( odbc )
	{
      // pop odbc should pop util a push?
		...
		{
			//DebugBreak();
			xlprintf(0)( "Warning! Popping a state which was not pushed! (should be a breakpoint here)" );
		}
		DestroyCollector( odbc->collection );
	}
}
...
SQLPROXY_PROC( void, PopODBC )( void )
{
   PopODBCEx( NULL );
}
...
int CPROC SQLServiceHandler( _32 SourceRouteID, _32 MsgID
