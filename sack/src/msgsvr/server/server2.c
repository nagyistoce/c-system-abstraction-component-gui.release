...
{
	_32 client_id; // process only? no thread?
	struct {
		_32 valid : 1;
		_32 error : 1; // uhmm - something like got a message from this but it wasn't known
		_32 status_queried : 1;
	} flags;
	int nClient; // unique routing ID of this client... services, clients, etc all ahve one.
	PLIST services;
...
static PCLIENT AddClient( _32 client_id )
{
	{
	...
	// cause nothing is reliable anymore under linux!
		...
		PCLIENT client = NULL; // FindClient( pid );
		if( client )
		{
...
static PSERVICE FindService( char *name )
{
	PSERVICE service;
...
static int CPROC MY_CLIENT_CONNECT( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
	PCLIENT client = AddClient( params[-1] );
	if( client )
	{
		// this is the ID that the client should use
		// to receive on.
		lprintf( WIDE("Client ID will be %d"), client->nClient );
		result[0] = client->nClient; // or something... hmmm....
		Log( WIDE("respondig success to client connect!") );
		(*result_length) = 4;
		return TRUE;
	}
	(*result_length) = 0;
...
static int CPROC MY_CLIENT_LOAD_SERVICE( _32 *params, _32 param_length
								  , _32 *result, _32 *result_length )
{
	...
	//PCLIENT client = FindClient( params[-1] );
	{
			...
			if( service->flags.bRemote )
		{
			...
			{
				params[-2 - ( sizeof( MSGIDTYPE)/4)] = service->client_id;
				// the real service will result to the client...
				// and further conversation will not involve me.
				if(SendOutMessage( (PQMSG)(params - 2 - (sizeof(MSGIDTYPE)/4)), param_length + 8 ) >= 0 )
				{
...
static int CPROC MY_CLIENT_REGISTER_SERVICE( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
...
// within this packet....  no... just a name I guess
	...
	// at least _32 to match the process identifier....
		...
		{
			if( strcmp( service->name, (char*)(params+1) ) == 0 )
				break;
		}
		...
		// for now - respond failure .
			service->first_message_index = params[0];
			if( ProbeClientAlive( service->client_id ) )
			{
				return FALSE;
			}
		...
		if( !service )
		{
			...
			service->client_id = params[-1];
			service->first_message_index = params[0];
			service->name = StrDup( (char*)(params + 1) );
...
static int CPROC MY_IM_ALIVE( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
	(*result_length) = INVALID_INDEX;
	return FALSE;
}
static int CPROC MY_RU_ALIVE( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
	(*result_length) = 0;
	return TRUE;
}
static int CPROC MY_CLIENT_DISCONNECT( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
	(*result_length) = INVALID_INDEX;
	return FALSE;
}
static int CPROC MY_CLIENT_UNLOAD_SERVICE( _32 *params, _32 param_length
	 , _32 *result, _32 *result_length )
{
	(*result_length) = INVALID_INDEX;
	return FALSE;
...
static int CPROC MY_CLIENT_LIST_SERVICES( _32 *params, _32 param_length
													 , _32 *result, _32 *result_length )
{
	...
	for( service = g.services; service; service = service->next )
	{
		SendMultiServiceEvent( params[-1], MSG_SERVICE_DATA
									, 3
	...
	SendMultiServiceEvent( params[-1], MSG_SERVICE_NOMORE, 1
								, &msg_data->bDone
