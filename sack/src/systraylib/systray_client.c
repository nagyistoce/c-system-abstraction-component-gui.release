...
static int CPROC HandleEvents( _32 source, _32 MsgID
										 ...
										 /*, _32 *result, _32 *result_length*/ )
{
   int retval;
	...
	case MSG_MateStarted:
		lprintf(WIDE("EventHandler <HandleEvents>.:MateStarted , client %") _32f WIDE(" is connecting to %s"), source, (char*)params );
...
static int ConnectToServer( void )
{
   ...
   if( !g.systrayserver.flags.connected )
   {
		...
		if( InitMessageService() )
      {
         ...
         if( g.systrayserver.MsgBase != INVALID_INDEX )
