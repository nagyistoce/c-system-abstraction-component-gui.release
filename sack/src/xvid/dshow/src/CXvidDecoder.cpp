    ...
    CVideoTransformFilter(NAME("CXvidDecoder"), punk, CLSID_XVID)
{
	...
	xvid_global_func = (int (__cdecl *)(void *, int, void *, void *))GetProcAddress(m_hdll, WIDE("xvid_global"));
	if (xvid_global_func == NULL) {
		MessageBox(0, WIDE("xvid_global() not found"), WIDE("Error"), 0);
		return;
	}

	xvid_decore_func = (int (__cdecl *)(void *, int, void *, void *))GetProcAddress(m_hdll, WIDE("xvid_decore"));
	...
	{
		MessageBox(0, WIDE("xvid_global() failed"), WIDE("Error"), 0);
		return;
	}
