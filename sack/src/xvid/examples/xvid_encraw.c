	 ...
	 char *argv[])
{
	...
	if(ME_ELEMENTS != VOP_ELEMENTS) {
		fprintf(stderr, WIDE("Presets' arrays should have the same number of elements -- Please fill a bug to xvid-devel@xvid.org\n"));
		return(-1);
	}
 ...
 ****************************************************************************/
	...
	if (ARG_FRAMERATE <= 0) {
		fprintf(stderr, WIDE("Wrong Framerate %s \n"), argv[5]);
		return (-1);
	}

	if (ARG_MAXFRAMENR <= 0) {
		fprintf(stderr, WIDE("Wrong number of frames\n"));
		return (-1);
	}

	if (ARG_INPUTFILE == NULL || strcmp(ARG_INPUTFILE, WIDE("stdin")) == 0) {
		in_file = stdin;
	} else {

		in_file = fopen(ARG_INPUTFILE, WIDE("rb"));
		if (in_file == NULL) {
			fprintf(stderr, WIDE("Error opening input file %s\n"), ARG_INPUTFILE);
			return (-1);
		}
	}
 ...
 ****************************************************************************/
	...
	if (result) {
		fprintf(stderr, WIDE("Encore INIT problem, return value %d\n"), result);
		goto release_all;
	}
 ...
 ****************************************************************************/
	...
	if (ARG_SAVEMPEGSTREAM && ARG_OUTPUTFILE) {
		...
		if ((out_file = fopen(ARG_OUTPUTFILE, WIDE("w+b"))) == NULL) {
			fprintf(stderr, WIDE("Error opening output file %s\n"), ARG_OUTPUTFILE);
			goto release_all;
		}
 ...
 ****************************************************************************/
		...
		if (m4v_size > 0 && ARG_SAVEMPEGSTREAM) {
			...
			if (ARG_SAVEINDIVIDUAL) {
				FILE *out;
				sprintf(filename, WIDE("%sframe%05d.m4v"), filepath, output_num);
				out = fopen(filename, WIDE("w+b"));
 ...
 ****************************************************************************/
  ...
  release_all:
	...
	if (enc_handle) {
		result = enc_stop();
		if (result)
			fprintf(stderr, WIDE("Encore RELEASE problem return value %d\n"),
					result);
	}
...
usage()
{
	fprintf(stderr, WIDE("Usage : xvid_stat [OPTIONS]\n\n"));
	fprintf(stderr, WIDE("Input options:\n"));
	fprintf(stderr, WIDE(" -i      string : input filename (default=stdin)\n"));
	fprintf(stderr, WIDE(" -type   integer: input data type (yuv=0, pgm=1)\n"));
	fprintf(stderr, WIDE(" -w      integer: frame width ([1.2048])\n"));
	fprintf(stderr, WIDE(" -h      integer: frame height ([1.2048])\n"));
	fprintf(stderr, WIDE(" -frames integer: number of frames to encode\n"));
	fprintf(stderr, WIDE("\n"));
	fprintf(stderr, WIDE("Output options:\n"));
	fprintf(stderr, WIDE(" -dump    : save decoder output\n"));
	fprintf(stderr, WIDE(" -save    : save an Elementary Stream file per frame\n"));
	fprintf(stderr, WIDE(" -o string: save an Elementary Stream for the complete sequence\n"));
	fprintf(stderr, WIDE("\n"));
	fprintf(stderr, WIDE("BFrames options:\n"));
	fprintf(stderr, WIDE(" -max_bframes   integer: max bframes (default=0)\n"));
	...
	fprintf(stderr, WIDE("\n"));
	fprintf(stderr, WIDE("Rate control options:\n"));
	fprintf(stderr, WIDE(" -framerate float               : target framerate (>0 | default=25.0)\n"));
	fprintf(stderr,	" -bitrate   integer             : target bitrate\n");
    fprintf(stderr,	" -single                        : single pass mode\n");
	fprintf(stderr, WIDE(" -pass1     filename            : twopass mode (first pass)\n"));
    ...
    fprintf(stderr, WIDE(" -max_key_interval integer      : maximum keyframe interval\n"));
    fprintf(stderr, WIDE("\n"));
	fprintf(stderr, WIDE("Other options\n"));
	fprintf(stderr, WIDE(" -asm            : use assembly optmized code\n"));
	fprintf(stderr, WIDE(" -quality integer: quality ([0..%d])\n"), ME_ELEMENTS - 1);
	fprintf(stderr, WIDE(" -qpel           : use quarter pixel ME\n"));
	fprintf(stderr, WIDE(" -gmc            : use global motion compensation\n"));
	fprintf(stderr, WIDE(" -packed         : packed mode\n"));
	fprintf(stderr, WIDE(" -closed_gop     : closed GOP mode\n"));
	fprintf(stderr, WIDE(" -lumimasking    : use lumimasking algorithm\n"));
	fprintf(stderr, WIDE(" -stats          : print stats about encoded frames\n"));
	fprintf(stderr, WIDE(" -debug          : activates xvidcore internal debugging output\n"));
	fprintf(stderr, WIDE(" -vop_debug      : print some info directly into encoded frames\n"));
	fprintf(stderr, WIDE(" -help           : prints this help message\n"));
	fprintf(stderr, WIDE("\n"));
	fprintf(stderr, WIDE("NB: You can define %d zones repeating the -z[qw] option as many times as needed.\n"), MAX_ZONES);
	fprintf(stderr, WIDE("\n"));
}
...
read_pgmheader(FILE * handle)
{
	...
	fscanf(handle, WIDE("%d %d %d"), &xsize, &ysize, &depth);
	if ((xsize > 4096) || (ysize > 4096*3/2) || (depth != 255)) {
		fprintf(stderr, WIDE("%d %d %d\n"), xsize, ysize, depth);
		return (2);
	}
	if ((XDIM == 0) || (YDIM == 0)) {
		XDIM = xsize;
