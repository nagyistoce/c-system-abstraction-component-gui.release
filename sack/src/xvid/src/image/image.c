...
/*
	if (interlacing)
		image_printf(image, edged_width, height, 5,5, WIDE("[i]"));
	image_dump_yuvpgm(image, edged_width, ((width+15)/16)*16, ((height+15)/16)*16, WIDE("\\encode.pgm"));
*/
	return 0;
}
...
/*
	if (interlacing)
		image_printf(image, edged_width, height, 5,100, WIDE("[i]=%i,%i"),width,height);
	image_dump_yuvpgm(image, edged_width, width, height, WIDE("\\decode.pgm"));
*/
...
int image_dump(IMAGE * image, uint32_t edged_width, uint32_t edged_height, char * path, int number)
{
	...
	sprintf(filename, WIDE("%s_%i_%c.pgm"), path, number, 'y');
	image_dump_pgm(
		image->y - (EDGE_SIZE * edged_width + EDGE_SIZE),
		edged_width, edged_height, filename);

	sprintf(filename, WIDE("%s_%i_%c.pgm"), path, number, 'u');
	image_dump_pgm(
		image->u - (EDGE_SIZE2 * edged_width / 2 + EDGE_SIZE2),
		edged_width / 2, edged_height / 2, filename);

	sprintf(filename, WIDE("%s_%i_%c.pgm"), path, number, 'v');
	image_dump_pgm(
