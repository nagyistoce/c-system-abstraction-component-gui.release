			  ...
			  const unsigned long best_sad)
{
	...
	if (((unsigned long) cur) & 0xf)
		fprintf(stderr, WIDE("sad16_altivec:incorrect align, cur: %x\n"), cur);
//  if (((unsigned long)ref) & 0xf)
//      fprintf(stderr, WIDE("sad16_altivec:incorrect align, ref: %x\n"), ref);
	if (stride & 0xf)
		fprintf(stderr, WIDE("sad16_altivec:incorrect align, stride: %x\n"), stride);
#endif
	/* initialization */
			 ...
			 unsigned long stride)
{
	...
	if (((unsigned long) cur) & 0x7)
		fprintf(stderr, WIDE("sad8_altivec:incorrect align, cur: %x\n"), cur);
//  if (((unsigned long)ref) & 0x7)
//      fprintf(stderr, WIDE("sad8_altivec:incorrect align, ref: %x\n"), ref);
	if (stride & 0xf)
		fprintf(stderr, WIDE("sad8_altivec:incorrect align, stride: %x\n"), stride);
#endif
