				...
				const int ref)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (mv[i].x > border) {
			DPRINTF(XVID_DEBUG_MV, WIDE("mv.x > max -- %d > %d, MB %d, %d"), mv[i].x, border, x_pos, y_pos);
			mv[i].x = border;
		} else {
			border = (-(int)x_pos-1) << (5 + dec->quarterpel);
			if (mv[i].x < border) {
				DPRINTF(XVID_DEBUG_MV, WIDE("mv.x < min -- %d < %d, MB %d, %d"), mv[i].x, border, x_pos, y_pos);
				mv[i].x = border;
			}
		}
		...
		if (mv[i].y >  border) {
			DPRINTF(XVID_DEBUG_MV, WIDE("mv.y > max -- %d > %d, MB %d, %d"), mv[i].y, border, x_pos, y_pos);
			mv[i].y = border;
		} else {
			border = (-(int)y_pos-1) << (5 + dec->quarterpel);
			if (mv[i].y < border) {
				DPRINTF(XVID_DEBUG_MV, WIDE("mv.y < min -- %d < %d, MB %d, %d"), mv[i].y, border, x_pos, y_pos);
				mv[i].y = border;
			}
		}
	}
				...
				int intra_dc_threshold)
{
	...
	for (y = 0; y < mb_height; y++) {
		for (x = 0; x < mb_width; x++) {
			...
			DPRINTF(XVID_DEBUG_MB, WIDE("macroblock (%i,%i) %08x\n"), x, y, BitstreamShowBits(bs, 32));
	...
	for (y = 0; y < mb_height; y++) {
		...
		for (x = 0; x < mb_width; x++) {
			...
			if (!(BitstreamGetBit(bs)))	{ /* block _is_ coded */
				...
				DPRINTF(XVID_DEBUG_MB, WIDE("mode %i\n"), mb->mode);
				DPRINTF(XVID_DEBUG_MB, WIDE("cbpc %i\n"), cbpc);
				...
				DPRINTF(XVID_DEBUG_MB, WIDE("cbpy %i mcsel %i \n"), cbpy,mcsel);

				cbp = (cbpy << 2) | cbpc;

				if (mb->mode == MODE_INTER_Q || mb->mode == MODE_INTRA_Q) {
					int dquant = dquant_table[BitstreamGetBits(bs, 2)];
					DPRINTF(XVID_DEBUG_MB, WIDE("dquant %i\n"), dquant);
					quant += dquant;
					if (quant > 31) {
						quant = 31;
					} else if (quant < 1) {
					...
					DPRINTF(XVID_DEBUG_MB, WIDE("quant %i\n"), quant);
				}
				mb->quant = quant;
				...
				if (dec->interlacing) {
					...
					if ((mb->mode == MODE_INTER || mb->mode == MODE_INTER_Q) && !mcsel) {
						...
						DPRINTF(XVID_DEBUG_MB, WIDE("decp: field_pred: %i\n"), mb->field_pred);
...
repeat:
	...
	DPRINTF(XVID_DEBUG_HEADER, WIDE("coding_type=%i,  packed=%i,  time=%lli,  time_pp=%i,  time_bp=%i\n"),
	...
	if (dec->packed_mode && coding_type == N_VOP) {
		...
		if (dec->low_delay) {
			DPRINTF(XVID_DEBUG_ERROR, WIDE("warning: bvop found in low_delay==1 stream\n"));
			dec->low_delay = 1;
		}
