...
#undef _INIT
	...
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Frame rate: %d/%d (%ffps)\n"),
			create->fbase, create->fincr,
			(double)create->fbase/(double)create->fincr);
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Number of frames: %d\n"), rc->num_frames);
	if(rc->param.bitrate>=0)
		DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Target bitrate: %ld\n"), rc->param.bitrate);
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Target filesize: %lld\n"), rc->target);

	/* Compensate the average frame overhead caused by the container */
	rc->target -= rc->num_frames*rc->param.container_frame_overhead;
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Container Frame overhead: %d\n"), rc->param.container_frame_overhead);
	if(rc->param.container_frame_overhead)
		DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- New target filesize after container compensation: %lld\n"), rc->target);
...
{
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- target_total:%lld desired_total:%.2f (%.2f%%) actual_total:%.2f (%.2f%%)\n"),
			rc->target,
			rc->desired_total,
...
rc_2pass2_after(rc_2pass2_t * rc, xvid_plg_data_t * data)
{
	...
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- frame:%d type:%c quant:%d stats:%d scaled:%d desired:%d actual:%d error:%d overflow:%.2f\n"),
			data->frame_num,
...
statsfile_count_frames(rc_2pass2_t * rc, char * filename)
{
	...
	if ((f = fopen(filename, WIDE("rb"))) == NULL)
		return(-1);
...
first_pass_stats_prepare_data(rc_2pass2_t * rc)
{
	...
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Min 1st pass IFrame length: %d\n"), rc->min_length[0]);
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Min 1st pass PFrame length: %d\n"), rc->min_length[1]);
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Min 1st pass BFrame length: %d\n"), rc->min_length[2]);
}
...
scaled_curve_apply_advanced_parameters(rc_2pass2_t * rc)
{
	...
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- IFrame boost tax ratio:%.2f\n"),
			rc->pb_iboost_tax_ratio);
	...
	DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- Assymetric tax ratio:%.2f\n"), rc->assymetric_tax_ratio);
...
stats_print(rc_2pass2_t * rc)
{
	...
	for (i=0; i<rc->num_frames; i++) {
		twopass_stat_t *s = &rc->stats[i];
		DPRINTF(XVID_DEBUG_RC, WIDE("[xvid rc] -- frame:%d type:%c quant:%d stats:%d scaled:%d desired:%d actual:%d overflow(%c):%.2f\n"),
