			 ...
			 int16_t data[6 * 64])
{
	...
	transfer_operation_8to16_t * functions[2];
	transfer_operation_8to16_t *transfer_op = NULL;
		{
			functions[0] = (transfer_operation_8to16_t *)transfer_8to16copy;
			functions[1] = (transfer_operation_8to16_t *)filter_18x18_to_8x8;
		};
			 ...
			 const uint8_t cbp)
{
	...
	transfer_operation_16to8_t  * functions[4];
	transfer_operation_16to8_t *transfer_op = NULL;
		{
			...
			functions[2] = (transfer_operation_16to8_t*)copy_upsampled_8x8_16to8;
			functions[3] = (transfer_operation_16to8_t*)add_upsampled_8x8_16to8;
		};
									   ...
									   const uint16_t * Zigzag, int Max, int Lambda)
{
	...
	if (DBG==1)
		printf( WIDE(" Last:%2d/%2d Cost = [(Bits=%5.0d) + Lambda*(Dist=%6.0d) = %d ] >>12= %d "), Last,Max, Bits, Dist, Cost, Cost>>12 );
	return Cost;
...
dct_quantize_trellis_h263_c(int16_t *const Out, const int16_t *const In, int Q, const uint16_t * const Zigzag, int Non_Zero)
{
	...
	for(i=0; i<=Non_Zero; i++)
	{
		...
		if ((uint32_t)(Level1+1)<3)                 /* very specialized loop for -1,0,+1 */
		{
			...
			if (DBG==1) {
				Run_Costs[i] = Best_Cost;
				printf( WIDE("Costs #%2d: "), i);
				for(j=-1;j<=Non_Zero;++j) {
					if (j==Run_Start)            printf( WIDE(" %3.0d|"), Run_Costs[j]>>12 );
					else if (j>Run_Start && j<i) printf( WIDE(" %3.0d|"), Run_Costs[j]>>12 );
					else if (j==i)               printf( WIDE("(%3.0d)"), Run_Costs[j]>>12 );
					else                         printf( WIDE("  - |") );
				}
				printf( WIDE("<%3.0d %2d %d>"), Min_Cost>>12, Nodes[i].Level, Nodes[i].Run );
				printf( WIDE("  Last:#%2d {%3.0d %2d %d}"), Last_Node, Last_Cost>>12, Last.Level, Last.Run );
				printf( WIDE(" AC:%3.0d Dist0:%3d Dist(%d)=%d"), AC, Dist0>>12, Nodes[i].Level, Cost0>>12 );
				printf( WIDE("\n") );
			}
		}
		else                      /* "big" levels */
 ...
 */
			...
			if (DBG==1) {
				...
				printf( WIDE("<%3.0d %2d %d>"), Min_Cost>>12, Nodes[i].Level, Nodes[i].Run );
				printf( WIDE("  Last:#%2d {%3.0d %2d %d}"), Last_Node, Last_Cost>>12, Last.Level, Last.Run );
				printf( WIDE(" AC:%3.0d Dist0:%3d Dist(%2d):%3d Dist(%2d):%3d"), AC, Dist0>>12, Level1, Dist1>>12, Level2, Dist2>>12 );
				printf( WIDE("\n") );
			}
		}
	...
	if (DBG) {
		...
		if (DBG==1) {
			printf( WIDE("=> ") );
			for(i=0; i<=Non_Zero; ++i) printf( WIDE("[%3.0d] "), Out[Zigzag[i]] );
			printf( WIDE("\n") );
		}
	}
	...
	if (DBG) {
		...
		if (DBG==1) {
			printf( WIDE("<= ") );
			for(i=0; i<=Last_Node; ++i) printf( WIDE("[%3.0d] "), Out[Zigzag[i]] );
			printf( WIDE("\n--------------------------------\n") );
		}
		if (Cost>Last_Cost) printf( WIDE("!!! %u > %u\n"), Cost, Last_Cost );
	}
	return Last_Node;
}
