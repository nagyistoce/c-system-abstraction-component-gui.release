...
repeat:
	...
	if (pEnc->queue_size == 0)		/* empty */
	{
		if (xFrame->input.csp == XVID_CSP_NULL)	/* no futher input */
		{
			...
			DPRINTF(XVID_DEBUG_DEBUG, WIDE("*** END\n"));
	...
	if (type == B_VOP) {
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width, pEnc->mbParam.height, 5, 200, WIDE("BVOP"));
		}
	...
	if (type == I_VOP && (pEnc->mbParam.global_flags & XVID_GLOBAL_CLOSED_GOP) && pEnc->bframenum_tail > 0) {
		...
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width, pEnc->mbParam.height, 5, 100, WIDE("DX50 BVOP->PVOP"));
		}
	...
	if (type == I_VOP) {
		...
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width, pEnc->mbParam.height, 5, 200, WIDE("IVOP"));
		}
	 ...
	 * %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
		...
		if ((pEnc->current->vop_flags & XVID_VOP_DEBUG)) {
			image_printf(&pEnc->current->image, pEnc->mbParam.edged_width, pEnc->mbParam.height, 5, 200, WIDE("PVOP"));
		}
...
/* This is a candidate for SKIPping, but for P-VOPs check intermediate B-frames first */
	...
	/* frame drop code */
#if 0
	DPRINTF(XVID_DEBUG_DEBUG, WIDE("kmu %i %i %i\n"), current->sStat.kblks, current->sStat.mblks, current->sStat.ublks);
#endif
	...
	{
		char s[100];
		sprintf(s, WIDE("\\%05i_cur.pgm"), pEnc->m_framenum);
		image_dump_yuvpgm(&current->image,
			pParam->edged_width,
			pParam->width, pParam->height, s);

		sprintf(s, WIDE("\\%05i_ref.pgm"), pEnc->m_framenum);
