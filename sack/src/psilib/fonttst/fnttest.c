...
int main( int argc, char **argv )
{
	...
	if( argc < 6 )
	{
		if( (argc > 1) && (strcmp( argv[1], WIDE("pick") ) == 0) )
		{
			FILE *in;
	...
	else 
	{
      ...
      printf( WIDE("Using parameters specified from command line ... %s %sx%s %s bits"), argv[2], argv[3], argv[4], argv[5] );
		if( bits == 8 )
			bits = 3;
		else if( bits == 2 )
			bits = 2;
