      ...
      if( ptc->background )
		...
		case 3: // right align - default to left align for now.
			if( pc->flags.bVertical )
				PutStringVerticalFont( surface
											, ( surface->width - width ) / 2, ( surface->height - height )
											, ptc->foreground, 0
											, GetText( pc->caption.text ), font );
			else
				PutStringFont( surface
								 , ptc->offset + surface->width - width, ( surface->height - height ) / 2
								 , ptc->foreground, 0
								 , GetText( pc->caption.text ), font );
			break;
		default:
		...
		case 1: // left
			if( pc->flags.bVertical )
				...
				PutStringFont( surface
								 , ptc->offset, ( surface->height - height ) / 2
								 , ptc->foreground
								 , ptc->background
								 , GetText( pc->caption.text ), font );
			break;
		case 2: // center
		{
			...
			if( pc->flags.bVertical )
				PutStringVerticalFont( surface
											...
											, ptc->offset + ( surface->height - width ) / 2
											, ptc->foreground
											, ptc->background
											, GetText( pc->caption.text ), font );
			else
