...
PSI_PROC( int, FrameBorderXOfs )( _32 BorderType )
{
...
PSI_PROC( int, FrameBorderX )( _32 BorderType )
{
...
void DrawFrameCaption( PCOMMON pc )
{
   ...
   if( pc->flags.bInitial || pc->flags.bHidden ) return;
	if( pc->BorderType & BORDER_NOCAPTION ) return;
	{
		int h, w;
		_32 width, height;
		_32 xofs = ( ( FrameBorderXOfs(pc->BorderType) * 3 ) / 4);
...
void UpdateSurface( PCOMMON pc )
{
	...
	pc->surface_rect.width = width - FrameBorderX(border);
