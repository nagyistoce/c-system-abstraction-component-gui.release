...
typedef struct SimpleDataBlock {
   PTRSZVAL size;/* unsigned size; size is sometimes a pointer value... this
...
{
	_32     Top; /* enable logging the program executable (probably the same for
	                all messages, unless they are network)
	                                                                             */
	_32     Cnt; // How many elements are on the stack. 
	_32     Lock;  /* thread interlock using InterlockedExchange semaphore. For
	                  thread safety.                                            */
	_32     Size; /* Size of each element in the stack. */
