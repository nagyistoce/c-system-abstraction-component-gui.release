...
#define my_va_arg(ap,type)     ((ap)[0]+=\
        ((sizeof(type)+ARG_STACK_SIZE-1)&~(ARG_STACK_SIZE-1)),\
...
CONFIGSCR_PROC( void, EncodeBinaryConfig )( TEXTSTR *encode, POINTER data, _32 length );
// this isn't REALLY the same function that's used, but serves the same purpose...
CONFIGSCR_PROC( int, DecodeBinaryConfig )( CTEXTSTR String, POINTER *binary_buffer, _32 *buflen );
