     ...
     char **makefiles;
{
  ...
  define_variable ("MAKEFILE_LIST", sizeof ("MAKEFILE_LIST")-1, WIDE(""), o_file, 0);
  ...
  if (num_makefiles == 0)
	...
	{ "makefile.vms", WIDE("gnumakefile."), WIDE("makefile."), 0 };
#else
#ifdef _AMIGA
	{ "GNUmakefile", WIDE("Makefile"), WIDE("SMakefile"), 0 };
#else /* !Amiga && !VMS */
	{ "GNUmakefile", WIDE("makefile"), WIDE("Makefile"), 0 };
#endif /* AMIGA */
#endif /* VMS */
     ...
     int flags;
{
  ...
  if (ebuf.fp == 0 && (flags & RM_INCLUDED) && *filename != '/')
	...
	{
	  char *name = concat (include_directories[i], WIDE("/"), filename);
	  ebuf.fp = fopen (name, WIDE("r"));
	  if (ebuf.fp == 0)
	    free (name);
	  else
	    {
  ...
  do_variable_definition (&ebuf.floc, WIDE("MAKEFILE_LIST"), filename, o_file,
                          f_append, 0);
              ...
              else
                {
                  ...
                  for (p = find_next_token (&p2, &len); p != 0;
                       p = find_next_token (&p2, &len))
                    {
                      ...
                      if (v == 0)
                        v = define_variable_loc (p, len, WIDE(""), o_file, 0,
                                                 fstart);
                      v->export = v_export;
                    }
          ...
          else
            {
              ...
              for (p = find_next_token (&p2, &len); p != 0;
                   p = find_next_token (&p2, &len))
                {
                  v = lookup_variable (p, len);
                  if (v == 0)
                    v = define_variable_loc (p, len, WIDE(""), o_file, 0, fstart);
	  ...
	  while (files != 0)
	    {
	      ...
	      if (!r)
                {
                  if (!noerror)
                    error (fstart, WIDE("%s: %s"), name, strerror (errno));
                  free (name);
                }
	    }
...

/* Interpret conditional commands "ifdef", WIDE("ifndef"), WIDE("ifeq"),
   "ifneq", WIDE("else") and "endif".
 ...
 */
  ...
  /* Look for multi-word archive references.
     ...
     with an unmatched `(' (e.g., WIDE("lib(mem")).  */
  ...
  while (new1 != 0)
	...
	if (n != 0
	    ...
	    && n->name[0] != '(')
	  {
		...
		name = concat (libname, paren, WIDE(")"));
		free (n->name);
		n->name = name;
	      }
		...
		name = concat (libname, new1->name, WIDE(""));
		free (new1->name);
		new1->name = name;
		new1 = new1->next;
	      }
     ...
     char **arg_dirs;
{
...
#ifdef  __MSDOS__
  {
	...
	strcat (strcpy (defdir, djdir->value), WIDE("/include"));
	dirs[idx++] = defdir;
      }
  }
#endif
     ...
     char *name;
{
  ...
  if (name[1] == '/' || name[1] == '\0')
      ...
      if (home_dir == 0 || home_dir[0] == '\0')
	{
      ...
      if (home_dir != 0)
	{
	  char *new = concat (home_dir, WIDE(""), name + 1);
	  if (is_variable)
	    free (home_dir);
	  return new;
	}
	...
	*userend = '\0';
      ...
      if (pwent != 0)
	{
	  if (userend == 0)
	    ...
	    return concat (pwent->pw_dir, WIDE("/"), userend + 1);
	}
      else if (userend != 0)
	*userend = '/';
    }
