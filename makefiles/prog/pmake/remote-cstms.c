     ...
     int *used_stdin;
{
  ...
  if (retsock < 0)
    {
      error (NILF, WIDE("exporting: Couldn't create return socket."));
      return 1;
    }
  ...
  if (getsockname (retsock, (struct sockaddr *) &sin, &len) < 0)
    {
      (void) close (retsock);
      perror_with_name ("exporting: ", WIDE("getsockname"));
      return 1;
    }
  ...
  if (status != RPC_SUCCESS)
    {
      (void) close (retsock);
      (void) close (sock);
      error (NILF, WIDE("exporting to %s: %s"),
    ...
    {
      error (NILF, WIDE("*** exported to %s (id %u)"),
	      host ? host->h_name : inet_ntoa (permit.addr),
	      permit.id);
    }
  ...
  if (pid < 0)
	...
	{ EXPORT_COMMAND, WIDE("-id"), sock_buf, retsock_buf, id_buf, 0 };

      /* Set up the arguments.  */
      (void) sprintf (sock_buf, WIDE("%d"), sock);
      (void) sprintf (retsock_buf, WIDE("%d"), retsock);
      (void) sprintf (id_buf, WIDE("%x"), permit.id);
