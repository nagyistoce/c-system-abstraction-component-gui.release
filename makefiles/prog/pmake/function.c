     ...
     register char *pattern_percent, *replace_percent;
{
  ...
  while ((t = find_next_token (&text, &len)) != 0)
	...
	{
	  o = variable_buffer_output (o, WIDE(" "), 1);
	  doneany = 1;
	}
    }
  if (doneany)
     ...
     const char *funcname;
{
  ...
  if (v == 0)
    o = variable_buffer_output (o, WIDE("undefined"), 9);
  else
    switch (v->origin)
      {
      default:
	...
	break;
      case o_default:
	o = variable_buffer_output (o, WIDE("default"), 7);
	break;
      case o_env:
	o = variable_buffer_output (o, WIDE("environment"), 11);
	break;
      case o_file:
	o = variable_buffer_output (o, WIDE("file"), 4);
	break;
      case o_env_override:
	o = variable_buffer_output (o, WIDE("environment override"), 20);
	break;
      case o_command:
	o = variable_buffer_output (o, WIDE("command line"), 12);
	break;
      case o_override:
	o = variable_buffer_output (o, WIDE("override"), 8);
	break;
      case o_automatic:
	o = variable_buffer_output (o, WIDE("automatic"), 9);
	break;
      }
     ...
     const char *funcname;
{
  ...
  int is_suffix = streq (funcname, WIDE("suffix"));
  ...
  while ((p2 = find_next_token (&list_iterator, &len)) != 0)
      ...
      if (p >= p2)
	{
      ...
      else if (streq (funcname, WIDE("notdir")) && p2[0] && p2[1] == ':')
	{
	  p = p2 + 2;
	  o = variable_buffer_output (o, p, len - (p - p2));
	}
     ...
     const char *funcname;
{
  ...
  unsigned int len=0;
  char *p=0;
  int is_basename= streq (funcname, WIDE("basename"));
...
#ifdef VMS
	    o = variable_buffer_output (o, WIDE("[]"), 2);
#else
#ifndef _AMIGA
	    o = variable_buffer_output (o, WIDE("./"), 2);
#else
     ...
     const char *funcname;
{
  ...
  int is_addprefix = streq (funcname, WIDE("addprefix"));
     ...
     const char *funcname;
{
  ...
  var = define_variable (varname, strlen (varname), WIDE(""), o_automatic, 0);
     ...
     const char *funcname;
{
  ...
  int is_filter = streq (funcname, WIDE("filter"));
     ...
     const char *funcname;
{
  ...
  if (*funcname == 'e')
    fatal (reading_file, WIDE("%s"), msg);

  /* The warning function expands to the empty string.  */
  error (reading_file, WIDE("%s"), msg);
  ...
  if (sh)
    {
	 ...
	 though the target environment is not passed to it.  */
      sprintf (buf, WIDE("SHELL=%s"), sh->value);
      putenv (buf);
    }
...
# ifdef __MSDOS__
  ...
  if (pipedes[0] < 0)
    {
      perror_with_name (error_prefix, WIDE("pipe"));
      return o;
    }
# else
  if (pipe (pipedes) < 0)
  ...
  pid = vfork ();
  if (pid < 0)
    perror_with_name (error_prefix, WIDE("fork"));
  else if (pid == 0)
...
#include <proto/dos.h>
  ...
  /* Note the mktemp() is a security hole, but this only runs on Amiga.
     ...
     with it.  */
  strcpy (tmp_output, WIDE("t:MakeshXXXXXXXX"));
  mktemp (tmp_output);
