...
{
  if (!silent_flag)
    message (0, WIDE("touch %s"), file->name);
	...
	file->update_status = 0;
      else
        {
          ...
          if (!keep_going_flag && !file->dontcare)
            {
              if (file->parent == 0)
                fatal (NILF, msg_noparent, WIDE(""), file->name, WIDE(""));

              fatal (NILF, msg_parent, WIDE(""), file->name, file->parent->name, WIDE(""));
            }

          if (!file->dontcare)
            {
              if (file->parent == 0)
                error (NILF, msg_noparent, WIDE("*** "), file->name, WIDE("."));
              else
                error (NILF, msg_parent, WIDE("*** "),
                       file->name, file->parent->name, WIDE("."));
            }
...
{
  struct stat st;
  lprintf( WIDE("findinthing thing of %s"), name );
  if (stat (name, &st) != 0)
    {
...
#define LIBDIR "."
  ...
  while ((p = find_next_token (&p2, &len)) != 0)
      ...
      else if (buflen < strlen (libbuf))
	{
      ...
      for (dp = dirs; *dp != 0; ++dp)
	{
	  sprintf (buf, WIDE("%s/%s"), *dp, libbuf);
