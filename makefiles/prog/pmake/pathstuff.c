...
convert_Path_to_windows32(char *Path, char to_delim)
{
    ...
    for (p = Path, etok = strpbrk(p, WIDE(":;"));
         etok;
         etok = strpbrk(p, WIDE(":;")))
        if ((etok - p) == 1) {
            if (*(etok - 1) == ';' ||
                ...
                *etok = to_delim;
                p = ++etok;
            } else if ((*etok == ':') && (etok = strpbrk(etok+1, WIDE(":;")))) {
...
convert_path_to_nutc(char *path)
{
    ...
    /* is this a multi-element path ? */
    for (p = path, etok = strpbrk(p, WIDE(":;")), count = 0;
         etok;
         etok = strpbrk(p, WIDE(":;")))
        if ((etok - p) == 1) {
            if (*(etok - 1) == ';' ||
                *(etok - 1) == ':') {
                p = ++etok;
                continue;    /* ignore empty bucket */
            } else if (etok = strpbrk(etok+1, WIDE(":;")))
                /* found one to count, handle drive letter */
                p = ++etok, count++;
            else
                /* all finished, force abort */
            ...
            if ((etok - p) == 1)
                if (*(etok - 1) == ';' ||
                    *(etok - 1) == ':') {
                    p = ++etok;
                    continue;
                } else if ((etok = strpbrk(etok+1, WIDE(":;"))) == NULL)
