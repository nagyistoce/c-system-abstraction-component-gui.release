  ...
  if (filename == NULL)
	      ...
	      && S_ISDIR (st.st_mode)))
	{
...
#else
	  {
	    ...
	    if (dircopy != NULL)
	      pglob->gl_pathv[pglob->gl_pathc] = (char*)memcpy (dircopy, dirname,
							 len);
	  }
#endif
      ...
      if (pglob->gl_pathc == oldcount)
	{
	  ...
	  if (flags & GLOB_NOCHECK)
	      ...
	      for (i = 0; i < dirs.gl_pathc; ++i)
		{
		  ...
		  pglob->gl_pathv[pglob->gl_pathc] = (char*)malloc (dir_len + 1
							     + filename_len);
		  if (pglob->gl_pathv[pglob->gl_pathc] == NULL)
		    {
		      globfree (&dirs);
  ...
  if (flags & GLOB_MARK)
	...
	if (((flags & GLOB_ALTDIRFUNC)
	    ...
	    && S_ISDIR (st.st_mode))
	  {
	    ...
	    char *_new_ = (char*)realloc (pglob->gl_pathv[i], len);
 	    if (_new_ == NULL)
	      {
		globfree (pglob);
		return GLOB_NOSPACE;
  ...
  for (i = 0; i < n; ++i)
    {
      ...
      char *_new_ = (char *) malloc (dirlen + 1 + eltlen);
      if (_new_ == NULL)
	{
	  while (i > 0)
...
#ifdef HAVE_MEMPCPY
      {
	char *endp = (char *) mempcpy (_new_, dirname, dirlen);
	*endp++ = DIRSEP_CHAR;
	mempcpy (endp, array[i], eltlen);
      }
#else
      memcpy (_new_, dirname, dirlen);
      _new_[dirlen] = DIRSEP_CHAR;
      memcpy (&_new_[dirlen + 1], array[i], eltlen);
#endif
      free ((__ptr_t) array[i]);
      array[i] = _new_;
    }
	  ...
	  if (stream == NULL)
	      ...
	      while (1)
		{
		  ...
		  if (fnmatch (pattern, name, fnm_flags) == 0)
		    {
		      struct globlink *_new_ = (struct globlink *)
			__alloca (sizeof (struct globlink));
		      len = NAMLEN (d);
		      _new_->name = (char *) malloc (len + 1);
		      if (_new_->name == NULL)
			goto memory_error;
#ifdef HAVE_MEMPCPY
		      *((char *) mempcpy ((__ptr_t) _new_->name, name, len))
			= '\0';
#else
		      memcpy ((__ptr_t) _new_->name, name, len);
		      _new_->name[len] = '\0';
#endif
		      _new_->next = names;
		      names = _new_;
