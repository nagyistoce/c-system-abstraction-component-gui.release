    ...
    char *ibuf;
{
  ...
  while (*ibuf && !isspace ((unsigned char)*ibuf))
    ibuf++;
  *ibuf = 0;
  if (strcmp (fptr, WIDE("/dev/null")) != 0)
    {
	  ...
	  coredump = 0;
...
#ifdef WINDOWS32
          {
            ...
            if (hPID = process_wait_for_any()) {
              ...
              if (err)
                fprintf(stderr, WIDE("make (e=%d): %s"),
...
#else
      (argv[0] && !strcmp (argv[0], WIDE("/bin/sh")))
#endif
      && (argv[1]
  ...
  if (getloadavg (&load, 1) != 1)
      ...
      if (lossage == -1 || errno != lossage)
	{
	  if (errno == 0)
	    ...
	    perror_with_name (_("cannot enforce load limit: "), WIDE("getloadavg"));
	}
      lossage = errno;
      load = 0;
    }
     ...
     struct child *child;
{
  ...
  sprintf (procname, WIDE("GMAKE_%05x"), getpid () & 0xfffff);
      ...
      switch (*p)
	{
	  ...
	  case '>':
	    ...
	    if (*(p-1) == '2')
	      {
		q--;
		if (strncmp (p, WIDE(">&1"), 3) == 0)
		  {
		    p += 3;
		    strcpy (efile, WIDE("sys$output"));
  ...
  *q = *p;
  ...
  if (strncmp (cmd, WIDE("builtin_"), 8) == 0)
    {
      child->pid = 270163;
      child->efn = 0;
      child->cstatus = 1;
  ...
  if (strlen (cmd) > MAXCMDLEN
      ...
      if (strlen (cmd) == 0)
	{
      ...
      outfile = open_tmpfile (&comname, WIDE("sys$scratch:CMDXXXXXX.COM"));
      if (outfile == 0)
	pfatal_with_name (_("fopen (temporary file)"));

      if (ifile[0])
	{
	  fprintf (outfile, WIDE("$ assign/user %s sys$input\n"), ifile);
      ...
      if (efile[0])
	{
	  fprintf (outfile, WIDE("$ define sys$error %s\n"), efile);
      ...
      if (ofile[0])
	{
	  fprintf (outfile, WIDE("$ define sys$output %s\n"), ofile);
  ...
  while (hWaitPID = process_wait_for_any())
    {
      ...
      if (err)
          fprintf(stderr, WIDE("make (e=%d, rc=%d): %s"),
     ...
     char **batch_filename_ptr;
{
  ...
  static char *sh_cmds_dos[] = { "break", WIDE("call"), WIDE("cd"), WIDE("chcp"), WIDE("chdir"), WIDE("cls"),
				 "copy", WIDE("ctty"), WIDE("date"), WIDE("del"), WIDE("dir"), WIDE("echo"),
				 "erase", WIDE("exit"), WIDE("for"), WIDE("goto"), WIDE("if"), WIDE("md"),
				 "mkdir", WIDE("path"), WIDE("pause"), WIDE("prompt"), WIDE("rd"),
				 "rmdir", WIDE("rem"), WIDE("ren"), WIDE("rename"), WIDE("set"),
				 "shift", WIDE("time"), WIDE("type"), WIDE("ver"), WIDE("verify"),
				 "vol", WIDE(":"), 0 };

  static char sh_chars_sh[]  = "#;\"*?[]&|<>(){}$`^";
  static char *sh_cmds_sh[]  = { "cd", WIDE("echo"), WIDE("eval"), WIDE("exec"), WIDE("exit"), WIDE("login"),
				 "logout", WIDE("set"), WIDE("umask"), WIDE("wait"), WIDE("while"),
				 "for", WIDE("case"), WIDE("if"), WIDE(":"), WIDE("."), WIDE("break"),
				 "continue", WIDE("export"), WIDE("read"), WIDE("readonly"),
				 "shift", WIDE("times"), WIDE("trap"), WIDE("switch"), WIDE("unset"),
                                 0 };
  ...
  static char *sh_cmds[] = { "cd", WIDE("eval"), WIDE("if"), WIDE("delete"), WIDE("echo"), WIDE("copy"),
			     "rename", WIDE("set"), WIDE("setenv"), WIDE("date"), WIDE("makedir"),
			     "skip", WIDE("else"), WIDE("endif"), WIDE("path"), WIDE("prompt"),
			     "unset", WIDE("unsetenv"), WIDE("version"),
			     0 };
#else
#ifdef WINDOWS32
  static char sh_chars_dos[] = "\"|&<>";
  static char *sh_cmds_dos[] = { "break", WIDE("call"), WIDE("cd"), WIDE("chcp"), WIDE("chdir"), WIDE("cls"),
			     "copy", WIDE("ctty"), WIDE("date"), WIDE("del"), WIDE("dir"), WIDE("echo"),
			     "erase", WIDE("exit"), WIDE("for"), WIDE("goto"), WIDE("if"), WIDE("if"), WIDE("md"),
			     "mkdir", WIDE("path"), WIDE("pause"), WIDE("prompt"), WIDE("rd"), WIDE("rem"),
                             "ren", WIDE("rename"), WIDE("rmdir"), WIDE("set"), WIDE("shift"), WIDE("time"),
                             "type", WIDE("ver"), WIDE("verify"), WIDE("vol"), WIDE(":"), 0 };
  static char sh_chars_sh[] = "#;\"*?[]&|<>(){}$`^";
  static char *sh_cmds_sh[] = { "cd", WIDE("eval"), WIDE("exec"), WIDE("exit"), WIDE("login"),
			     "logout", WIDE("set"), WIDE("umask"), WIDE("wait"), WIDE("while"), WIDE("for"),
			     "case", WIDE("if"), WIDE(":"), WIDE("."), WIDE("break"), WIDE("continue"),
			     "export", WIDE("read"), WIDE("readonly"), WIDE("shift"), WIDE("times"),
			     "trap", WIDE("switch"), WIDE("test"),
  ...
  static char sh_chars[] = "#;\"*?[]&|<>(){}$`^~";
  static char *sh_cmds[] = { "cd", WIDE("eval"), WIDE("exec"), WIDE("exit"), WIDE("login"),
			     "logout", WIDE("set"), WIDE("umask"), WIDE("wait"), WIDE("while"), WIDE("for"),
			     "case", WIDE("if"), WIDE(":"), WIDE("."), WIDE("break"), WIDE("continue"),
			     "export", WIDE("read"), WIDE("readonly"), WIDE("shift"), WIDE("times"),
			     "trap", WIDE("switch"), 0 };
 ...
 slow:;
...
#endif /* WINDOWS32 */
  {
	...
	if (unixy_shell && !batch_mode_shell &&
	  ...
	  *ap++ = '\\';
#ifdef __MSDOS__
        else if (unixy_shell && strneq (p, WIDE("..."), 3))
          {
      ...
      *batch_filename_ptr = xmalloc(strlen(fname) + 5);
      ...
      if (!unixy_shell) {
        ...
        for (i = 0; fname[i] != '\0'; ++i)
          if (fname[i] == '/')
            fname[i] = '\\';
        strcat(*batch_filename_ptr, WIDE(".bat"));
      } else {
        strcat(*batch_filename_ptr, WIDE(".sh"));
      }
      ...
      batch = fopen (*batch_filename_ptr, WIDE("w"));
      if (!unixy_shell)
        fputs ("@echo off\n", batch);
