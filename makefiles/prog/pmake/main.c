...
static const struct command_switch switches[] =
  {
    ...
    { 'B', flag, (char *) &always_make_flag, 1, 1, 0, 0, 0, WIDE("always-make") },
    { 'C', string, (char *) &directories, 0, 0, 0, 0, 0, WIDE("directory") },
    { 'd', flag, (char *) &debug_flag, 1, 1, 0, 0, 0, 0 },
    { CHAR_MAX+1, string, (char *) &db_flags, 1, 1, 0, WIDE("basic"), 0, WIDE("debug") },
#ifdef WINDOWS32
    { 'D', flag, (char *) &suspend_flag, 1, 1, 0, 0, 0, WIDE("suspend-for-debug") },
#endif
    { 'e', flag, (char *) &env_overrides, 1, 1, 0, 0, 0,
        "environment-overrides", },
    { 'f', string, (char *) &makefiles, 0, 0, 0, 0, 0, WIDE("file") },
    { 'h', flag, (char *) &print_usage_flag, 0, 0, 0, 0, 0, WIDE("help") },
    { 'i', flag, (char *) &ignore_errors_flag, 1, 1, 0, 0, 0,
        "ignore-errors" },
    { 'I', string, (char *) &include_directories, 1, 1, 0, 0, 0,
        "include-dir" },
    { 'j', positive_int, (char *) &job_slots, 1, 1, 0, (char *) &inf_jobs,
        (char *) &default_job_slots, WIDE("jobs") },
    { CHAR_MAX+2, string, (char *) &jobserver_fds, 1, 1, 0, 0, 0,
        "jobserver-fds" },
    { 'k', flag, (char *) &keep_going_flag, 1, 1, 0, 0,
        (char *) &default_keep_going_flag, WIDE("keep-going") },
#ifndef NO_FLOAT
    ...
    { 'n', flag, (char *) &just_print_flag, 1, 1, 1, 0, 0, WIDE("just-print") },
    { 'o', string, (char *) &old_files, 0, 0, 0, 0, 0, WIDE("old-file") },
    { 'p', flag, (char *) &print_data_base_flag, 1, 1, 0, 0, 0,
        "print-data-base" },
    { 'q', flag, (char *) &question_flag, 1, 1, 1, 0, 0, WIDE("question") },
    { 'r', flag, (char *) &no_builtin_rules_flag, 1, 1, 0, 0, 0,
      "no-builtin-rules" },
    { 'R', flag, (char *) &no_builtin_variables_flag, 1, 1, 0, 0, 0,
	"no-builtin-variables" },
    { 's', flag, (char *) &silent_flag, 1, 1, 0, 0, 0, WIDE("silent") },
    { 'S', flag_off, (char *) &keep_going_flag, 1, 1, 0, 0,
      (char *) &default_keep_going_flag, WIDE("no-keep-going") },
    { 't', flag, (char *) &touch_flag, 1, 1, 1, 0, 0, WIDE("touch") },
    { 'v', flag, (char *) &print_version_flag, 1, 1, 0, 0, 0, WIDE("version") },
    { 'w', flag, (char *) &print_directory_flag, 1, 1, 0, 0, 0,
        "print-directory" },
    { CHAR_MAX+3, flag, (char *) &inhibit_print_directory_flag, 1, 1, 0, 0, 0,
	"no-print-directory" },
    { 'W', string, (char *) &new_files, 0, 0, 0, 0, 0, WIDE("what-if") },
...
#ifdef USE_EVENT_LOG
  hEventSource = RegisterEventSource(NULL, WIDE("GNU Make"));
...
find_and_set_default_shell(char *token)
{
  ...
  if (!no_default_sh_exe &&
    ...
    sprintf(sh_path, WIDE("%s"), search_token);
      ...
      while (ep && *ep) {
        *ep = '\0';
        ...
        if (dir_file_exists_p(p, search_token)) {
          sprintf(sh_path, WIDE("%s/%s"), p, search_token);
          default_shell = xstrdup(w32ify(sh_path,0));
          sh_found = 1;
  ...
  if (!unixy_shell && sh_found &&
      (strstr(default_shell, WIDE("sh")) || strstr(default_shell, WIDE("SH")))) {
    unixy_shell = 1;
    batch_mode_shell = 0;
  }
...
# else
  ...
  return fopen (*name, WIDE("w"));
# endif
#endif
}


#if !defined( _AMIGA ) && !defined( __WATCOMC__ )
int
main (argc, argv, envp)
     int argc;
     char **argv;
...
# endif	/* setvbuf reversed.  */
  ...
  if (getcwd (current_directory, GET_PATH_MAX) == 0)
    ...
    {
#ifdef	HAVE_GETCWD
      perror_with_name ("getcwd: ", WIDE(""));
#else
      error (NILF, WIDE("getwd: %s"), current_directory);
#endif
  ...
  define_variable (".VARIABLES", 10, WIDE(""), o_default, 0)->special = 1;
  /* define_variable (".TARGETS", 8, WIDE(""), o_default, 0); */
  ...
  for (i = 0; envp[i] != 0; ++i)
    {
      ...
      while (*ep != '=')
        ++ep;
#ifdef WINDOWS32
      if (!unix_path && strneq(envp[i], WIDE("PATH="), 5))
        unix_path = ep+1;
      else if (!windows32_path && !strnicmp(envp[i], WIDE("Path="), 5)) {
    ...
    if (windows32_path)
      ...
      define_variable("Path", 4, WIDE(""), o_env, 1)->export = v_export;
    ...
    if (!unix_path && windows32_path)
      define_variable("PATH", 4, windows32_path, o_env, 1)->export = v_export;
#endif
#elif !defined( __WATCOMC__ ) /* For Amiga, read the ENV: device, ignoring all dirs */
    {
	BPTR env, file, old;
	char buffer[1024];
	int len;
  ...
  for (i = 0; environ[i] != 0; ++i)
    {
      ...
      while (*ep != '=')
        ++ep;
#ifdef WINDOWS32
      if (!unix_path && strneq(environ[i], WIDE("PATH="), 5))
        unix_path = ep+1;
      else if (!windows32_path && !strnicmp(environ[i], WIDE("Path="), 5)) {
        do_not_define = 1; /* it gets defined after loop exits */
        windows32_path = ep+1;
      }
#endif
	 ...
	 the same.  */
      if (!do_not_define)
        define_variable (environ[i], (unsigned int) (ep - environ[i]),
                         ep + 1, o_env, 1)
	/* Force exportation of every variable culled from the environment.
	   We used to rely on target_environment's v_default code to do this.
	   But that does not work for the case where an environment variable
   ...
   * CreateProcess().
   */
  if (strpbrk(argv[0], WIDE("/:\\")) ||
      strstr(argv[0], WIDE("..")) ||
      strneq(argv[0], WIDE("//"), 2))
  ...
  if (current_directory[0] != '\0'
    ...
    argv[0] = concat (current_directory, WIDE("/"), argv[0]);
#else  /* !__MSDOS__ */
  if (current_directory[0] != '\0'
  ...
  (void) define_variable ("MAKE", 4, WIDE("$(MAKE_COMMAND)"), o_default, 1);
...
# endif
  ...
  if (jobserver_fds)
  {
    ...
    if (sscanf (cp, WIDE("%d,%d"), &job_fds[0], &job_fds[1]) != 2)
      fatal (NILF,
  ...
  if (job_slots > 1)
	...
	pfatal_with_name (_("creating jobs pipe"));
      ...
      sprintf (jobserver_fds->list[0], WIDE("%d,%d"), job_fds[0], job_fds[1]);
      jobserver_fds->idx = 1;
      jobserver_fds->max = 1;
    }
#endif
  ...
  if (read_makefiles != 0)
	...
	re_exec:
	  ...
	  if (makefiles != 0)
	    {
	      ...
	      for (i = 1; i < argc; ++i)
		if (strneq (argv[i], WIDE("-f"), 2)) /* XXX */
		  {
		    char *p = &argv[i][2];
		    if (*p == '\0')
		      argv[++i] = makefiles->list[j];
		    else
		      argv[i] = concat ("-f", makefiles->list[j], WIDE(""));
		    ++j;
		  }
	    }
          ...
          if (stdin_nm)
            {
              ...
              nargv[nargc++] = concat ("-o", stdin_nm, WIDE(""));
              nargv[nargc] = 0;
            }
	  ...
	  for (p = environ; *p != 0; ++p)
		...
		*p = sgi_loses;
		sprintf (*p, WIDE("%s=%u"), MAKELEVEL_NAME, makelevel);
		break;
	      }
#else /* AMIGA */
	  {
	    ...
	    if (len != -1)
	    {
	    sprintf (buffer, WIDE("%u"), makelevel);
	...
	case positive_int:
	  if (all)
	      ...
	      else
		{
		  ...
		  sprintf (buf, WIDE("%u"), *(unsigned int *) cs->value_ptr);
	...
	case floating:
	  if (all)
	      ...
	      else
		{
		  ...
		  sprintf (buf, WIDE("%g"), *(double *) cs->value_ptr);
		  ADD_FLAG (buf, strlen (buf));
		}
	    }
	  break;
