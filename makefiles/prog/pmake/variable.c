     ...
     struct variable *var;
{
  ...
  if (streq (var->name, WIDE(".TARGETS")))
    var->value = build_target_list (var->value);
  else
  */

  if (streq (var->name, WIDE(".VARIABLES"))
...
define_automatic_variables ()
{
  ...
  v = define_variable ("MAKEFILES", 9, WIDE(""), o_default, 0);
  v->export = v_ifset;
...
#ifdef VMS
  define_variable ("@D", 2, WIDE("$(dir $@)"), o_automatic, 1);
  define_variable ("%D", 2, WIDE("$(dir $%)"), o_automatic, 1);
  define_variable ("*D", 2, WIDE("$(dir $*)"), o_automatic, 1);
  define_variable ("<D", 2, WIDE("$(dir $<)"), o_automatic, 1);
  define_variable ("?D", 2, WIDE("$(dir $?)"), o_automatic, 1);
  ...
  define_variable ("+D", 2, WIDE("$(dir $+)"), o_automatic, 1);
#else
  define_variable ("@D", 2, WIDE("$(patsubst %/,%,$(dir $@))"), o_automatic, 1);
  define_variable ("%D", 2, WIDE("$(patsubst %/,%,$(dir $%))"), o_automatic, 1);
  define_variable ("*D", 2, WIDE("$(patsubst %/,%,$(dir $*))"), o_automatic, 1);
  define_variable ("<D", 2, WIDE("$(patsubst %/,%,$(dir $<))"), o_automatic, 1);
  define_variable ("?D", 2, WIDE("$(patsubst %/,%,$(dir $?))"), o_automatic, 1);
  ...
  define_variable ("+D", 2, WIDE("$(patsubst %/,%,$(dir $+))"), o_automatic, 1);
#endif
  define_variable ("@F", 2, WIDE("$(notdir $@)"), o_automatic, 1);
  define_variable ("%F", 2, WIDE("$(notdir $%)"), o_automatic, 1);
  define_variable ("*F", 2, WIDE("$(notdir $*)"), o_automatic, 1);
  define_variable ("<F", 2, WIDE("$(notdir $<)"), o_automatic, 1);
  define_variable ("?F", 2, WIDE("$(notdir $?)"), o_automatic, 1);
...
#ifdef WINDOWS32
	    if (strcmp(v->name, WIDE("Path")) == 0 ||
		strcmp(v->name, WIDE("PATH")) == 0)
	      convert_Path_to_windows32(value, ';');
#endif
	    *result++ = concat (v->name, WIDE("="), value);
	    free (value);
	  }
	else
	  {
...
#endif
	    *result++ = concat (v->name, WIDE("="), v->value);
	  }
      }

  *result = (char *) xmalloc (100);
  (void) sprintf (*result, WIDE("%s=%u"), MAKELEVEL_NAME, makelevel + 1);
  *++result = 0;
     ...
     int target_var;
{
  ...
  if ((origin == o_file || origin == o_override)
      && strcmp (varname, WIDE("SHELL")) == 0)
    {
      char shellpath[PATH_MAX];
      extern char * __dosexec_find_on_path (const char *, char *[], char *);

      /* See if we can find "/bin/sh.exe", WIDE("/bin/sh.com"), etc.  */
      ...
      else
	{
	  ...
	  sprintf (path_string, WIDE("PATH=.;%s"), pathv ? pathv->value : "");
...
#endif /* __MSDOS__ */
#ifdef WINDOWS32
  if ((origin == o_file || origin == o_override) && streq (varname, WIDE("SHELL")))
    {
      extern char *default_shell;
...
print_variable_data_base ()
{
  ...
  print_variable_set (&global_variable_set, WIDE(""));
}
...
{
  if (file->variables != 0)
    print_variable_set (file->variables->set, WIDE("# "));
}
...
sync_Path_environment(void)
{
    ...
    environ_path = concat("Path", WIDE("="), path);
    putenv(environ_path);
    free(path);
}
#endif
