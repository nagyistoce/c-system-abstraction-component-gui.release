     ...
     long int arg;
{
  ...
  /* Now find the members one by one.  */
  {
    ...
    if ( big_archive )
      {
	sscanf (fl_header_big.fl_fstmoff, WIDE("%20ld"), &member_offset);
	sscanf (fl_header_big.fl_lstmoff, WIDE("%20ld"), &last_member_offset);
      }
    else
#endif
      {
	sscanf (fl_header.fl_fstmoff, WIDE("%12ld"), &member_offset);
	sscanf (fl_header.fl_lstmoff, WIDE("%12ld"), &last_member_offset);
      }
	...
	if (big_archive)
	  {
	    ...
	    sscanf (member_header_big.ar_namlen, WIDE("%4d"), &name_len);
	    ...
	    sscanf (member_header_big.ar_date, WIDE("%12ld"), &dateval);
	    sscanf (member_header_big.ar_uid, WIDE("%12d"), &uidval);
	    sscanf (member_header_big.ar_gid, WIDE("%12d"), &gidval);
	    sscanf (member_header_big.ar_mode, WIDE("%12o"), &eltmode);
	    sscanf (member_header_big.ar_size, WIDE("%20ld"), &eltsize);
...
#endif
	  {
	    ...
	    sscanf (member_header.ar_namlen, WIDE("%4d"), &name_len);
	    nread = read (desc, name, name_len);
	    ...
	    sscanf (member_header.ar_date, WIDE("%12ld"), &dateval);
	    sscanf (member_header.ar_uid, WIDE("%12d"), &uidval);
	    sscanf (member_header.ar_gid, WIDE("%12d"), &gidval);
	    sscanf (member_header.ar_mode, WIDE("%12o"), &eltmode);
	    sscanf (member_header.ar_size, WIDE("%12ld"), &eltsize);
	...
	bcopy (member_header.ar_name, name, sizeof member_header.ar_name);
	{
	  ...
	  /* If the member name is "//" or "ARFILENAMES/" this may be
	     a list of file name mappings.  The maximum file name
 	     ...
 	     that.  */
 	  is_namemap = (!strcmp (name, WIDE("//"))
			|| !strcmp (name, WIDE("ARFILENAMES/")));
#endif	/* Not AIAMAG. */
...
#ifndef	M_XENIX
	sscanf (member_header.ar_mode, WIDE("%o"), &eltmode);
...
#ifdef AIAMAGBIG
	if (big_archive)
          sscanf (member_header_big.ar_nxtmem, WIDE("%20ld"), &member_offset);
	else
#endif
	  sscanf (member_header.ar_nxtmem, WIDE("%12ld"), &member_offset);
     ...
     char *arname, *memname;
{
  ...
  for (i = 0; i < sizeof ar_hdr.ar_date; i++)
    ar_hdr.ar_date[i] = ' ';
  sprintf (ar_hdr.ar_date, WIDE("%ld"), (long int) statbuf.st_mtime);
#ifdef AIAMAG
